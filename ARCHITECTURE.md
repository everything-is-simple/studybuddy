...
启动时在数据库初始化后执行一次 `recovery.reconcile`：仅删除 data root 顶层遗留 `.incoming-*` 普通文件；仅对严格 `originals/<2 hex>/<62 hex>/original` candidate，在内容 hash 正确且无 active/deleted material 引用时删除 orphan。hash mismatch、unexpected layout 和 symlink 均保留，missing original 仅记录且不改 material。删除失败不会阻断启动，recovery 不使用 stored_path，不运行后台任务；多进程共享同一 data_root 不属于支持范围。写入失败会安全返回并清理临时文件/新建零引用 original，batch partial-success 边界保持。
