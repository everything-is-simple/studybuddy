...
StudyBuddy 当前是本地、单进程、单实例的 FastAPI + SQLite + 本地 hash-derived originals 系统，正式 schema 为 v13；10-3 已提供 explicit-only 单进程 task runner/recovery，10-4 仅接入 approved `embedding_index` provider-backed task，10-5 已实现 safe structured observability、health/readiness/degraded 与 explicit read-only diagnostics，10-6 已完成 non-mutating upgrade preflight、verified rotation、restore drill 与 stop/quarantine policy，10-7 已完成 safe local runtime configuration、single-instance lock 和 Windows start/health/stop scripts；runner 仍需显式 API/CLI 调用，startup/backup/restore/read 不会自动启动或执行。Q&A、generation、OCR/ASR、report/delivery 未接入。Phase 4–8 及 Phase 9A/9B/9C、Phase 9D 部分立项范围已在各自限定范围完成，但项目尚未全局 production real-pass。当前支持边界是单进程、单实例、本地磁盘；不得把 Phase 10 自动解释为多 worker、多实例、多用户、云同步或互联网 SaaS。
