# Phase 9A 领域契约与现状审计（9A-0 至 9A-8 记录）

> 状态：9A-0 `audit-draft`、9A-1 `contract-frozen`、9A-2/9A-3/9A-4 `implemented/backend-pass`、9A-5 `browser-pass`、9A-6 `scoped-gates-pass`、9A-7 `restore-gates-pass`、9A-8 `completed`（限定范围）。
>
> 本文记录当前正式代码审计、Phase 9A 的边界以及已冻结的领域契约。9A-2 至 9A-8 已形成 schema、repository/domain、API、source lifecycle、backup/restore、本地 Chromium workspace 和最终 acceptance 的 scoped evidence。
>
...
- 9A 当前已有学习目标、知识模块、study plan/item、dependency、progress repository、API、source lifecycle、backup/restore 和本地 Chromium workspace 的实现；完整范围与限制见 `docs/PHASE9A_ACCEPTANCE_EVIDENCE.md`。不能把历史版本实现当作正式系统证据。
...
`Phase 9A-6 scoped-gates-pass`：材料 delete/restore/purge/re-index 已与 9A source links 接入；restore 保持 source status，显式 refresh 才重算；purge 保留 plan/progress/link 历史并固定 unavailable；active plan 在 source warning 下保持 active；module/item link、progress projection 和 source privacy 已覆盖。focused backend `16 passed`、full backend `270 passed, 2 skipped`、Phase 9A Chromium `3 passed`，脱敏 scoped evidence 见 `docs/prompts/evidence/PHASE9A_SOURCE_LIFECYCLE_EVIDENCE.md`；最终 acceptance 见 `docs/PHASE9A_ACCEPTANCE_EVIDENCE.md`。

`Phase 9A-7 restore-gates-pass`：现有 SQLite Online Backup、manifest、schema history 和新空 data root restore 已覆盖全部 9A 表。新增 restore acceptance 验证 plan status、item projection、dependency、append-only progress、summary、user-edited 和 valid/stale/source_unavailable link 状态；restore 会重定位恢复目标的 material stored path，但不生成业务对象、不重建派生数据、不调用 Provider、不 repair source lifecycle。focused backup/restore `13 passed`、full backend `272 passed, 2 skipped`，脱敏 evidence 见 `docs/prompts/evidence/PHASE9A_BACKUP_RESTORE_EVIDENCE.md`。

`Phase 9A-8 completed`：完整 backend、9A focused backend、Phase 9A Chromium、Phase 8 Chromium、frontend failure contract Chromium 和脱敏 acceptance evidence 已通过。准确完成声明及 not_verified 边界见 `docs/PHASE9A_ACCEPTANCE_EVIDENCE.md`。

### 下一步

进入 Phase 9B；不把 9A 完成扩大解释为 Phase 9B–9D 或 Phase 9 全部完成。
