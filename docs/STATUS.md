...
| Phase 6 P6-E core workflow acceptance | fake-Provider-tested / backend-tested / real UI not_verified | `backend/tests/browser_p6e.spec.js`, `backend/tests/browser_p6e_real_provider.spec.js`, `docs/P6E_ACCEPTANCE_EVIDENCE.md`; complete import → indexing → retrieval → thread → Q&A → citation → body location → material/Q&A return → export → refresh/history path, empty retrieval, unconfigured Provider, timeout/retry, duplicate click, stale thread response, deleted source/export safety and related failure contracts are covered; focused P6-E 4 passed, related Chromium 19 passed/3 skipped, focused backend 47 passed/2 skipped, full backend 200 passed/2 skipped; DeepSeek `deepseek-chat` and Agnes `agnes-ai-hub`/`agnes-2.5-flash` real UI paths were not run this turn |
