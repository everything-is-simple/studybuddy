...
| Phase 5 OpenAI-compatible adapter | implemented / mock-tested / DeepSeek API+UI smoke passed; target-gated other-provider validation pending; Agnes local launcher added | `backend/app/providers.py`, `backend/scripts/agnes-*.ps1`, `backend/tests/test_agnes_launcher.py`, `backend/tests/test_phase5_provider.py`, `backend/tests/test_real_provider_smoke.py`, `backend/tests/browser_qa.spec.js`; Agnes remains not_verified without matching runtime evidence |
