...
| Phase 5 OpenAI-compatible adapter | implemented / mock-tested / DeepSeek API-smoke passed; general real-pass pending | `backend/app/providers.py`, `backend/tests/test_phase5_provider.py`, `backend/tests/test_real_provider_smoke.py`; DeepSeek `deepseek-chat` synthetic API path passed; UI/E2E and other providers pending |
...
- Material revision, explicit deterministic chunk indexing, lexical chunk retrieval, context assembly with citation contract, deterministic fake provider, synchronous Q&A API/persistence, Q&A history, multi-material scope, citation detail/navigation and the Phase 4 full-path browser E2E are implemented and verified. Phase 5 now has a tested OpenAI-compatible adapter, v3 provider metadata migration, and a DeepSeek `deepseek-chat` adapter/API-level synthetic smoke pass; browser provider E2E, real failure UX and other provider-specific validation remain pending. Cards, exercises and study plans are not implemented.
