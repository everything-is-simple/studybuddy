...
| Phase 7.2–7.6 embedding/indexing/retrieval/Q&A integration | implemented / backend-tested | versioned deterministic fake provider, canonical identity/codec/stale checks, explicit indexing/rebuild/verify, vector cosine, deterministic hybrid RRF, explicit fallback, Q&A lexical/vector/hybrid mode selection, operation/retrieval linkage, replay metadata and citation-safe context are implemented; retrieval mode UI, real embedding adapter and embedding/retrieval-specific backup evidence remain incomplete |
