...
| Phase 5 OpenAI-compatible adapter | implemented / mock-tested / DeepSeek API+UI smoke passed; other-provider validation pending | `backend/app/providers.py`, `backend/tests/test_phase5_provider.py`, `backend/tests/test_real_provider_smoke.py`, `backend/tests/browser_qa.spec.js`; DeepSeek `deepseek-chat` API and Chromium UI paths passed |
...
- Metrics are process-local, reset on restart, and do not provide cross-process aggregation; operation IDs are request-scoped correlation only. `ai_operations.input_fingerprint` is metadata only; true duplicate-request idempotency and stale transitions are not implemented.
- Material revision, explicit deterministic chunk indexing, lexical chunk retrieval, context assembly with citation contract, deterministic fake provider, synchronous Q&A API/persistence, Q&A history, multi-material scope, citation detail/navigation and the Phase 4 full-path browser E2E are implemented and verified. Phase 5 now has a tested OpenAI-compatible adapter, v3 provider metadata migration, and DeepSeek `deepseek-chat` adapter/API-level/Chromium UI synthetic smoke passes; other provider-specific validation remains pending. True operation idempotency, duplicate-request deduplication and stale transitions are not implemented. Cards, exercises and study plans are not implemented.
