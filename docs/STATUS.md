...
| Phase 7.2–7.5 embedding/indexing/retrieval foundation | implemented / backend-tested | versioned deterministic fake provider, independent registry/config, canonical identity, f32le_v1 codec, stale/source binding checks, ready-only filtering, idempotent indexing/rebuild/verify, vector cosine, deterministic hybrid RRF and explicit lexical fallback are implemented; real network adapter, Q&A/citation/UI integration and embedding/retrieval-specific backup evidence remain incomplete |
