...
| P14-P1-04 | 来源状态在不同页面使用 `source_status`、`source_link_status`、`source_citation_status` 等多个字段名。**C0 已实测确认**：计划学习项响应没有任何来源状态字段（只存在于 `source_links`），因此 `plans.html` 与 `today.html` 在来源已删除时仍显示“来源有效”；`not_indexed` 在 `state.js` 也无文案。 | L2 | B1-3、B2-4~B2-6、B3-3、B4-2、B5-5 | P1 | **C2 已修复**：不改既有响应字段名，`state.js` 从 plan 的 `source_links` 按 item/module 映射状态；无链接显示“未关联来源”，非 valid 来源禁用进入材料详情；补齐 `not_indexed` 文案。 | `test_p1_4_c2_explainability.py` 4 passed；`browser_p1_4_c2_explainability.spec.js` 真实 valid→source_deleted→restart 路径 1 passed；关联矩阵 7 passed。结论为 L2/L3（限定 deterministic fake provider、本地单进程、隔离 data root）。 |
| P14-P1-05 | 解析结果对用户不够可解释：PDF 无文字层、DOCX 复杂对象未提取、PPTX 图片页等情况虽有 warning/empty 语义，但没有通过真实样本确认页面是否足够指导下一步。 | L2 | B1-3、B2-1~B2-6 | P1 | **C2 已修复**：材料详情展示解析状态/解析器/安全 warning，并对 image-only PDF、空 PPTX/正文和失败/拒绝结果提供下一步提示；不把提示改成虚假的成功。 | `browser_p1_4_c2_explainability.spec.js` 使用真实 Chromium 生成 PDF/DOCX/PPTX，2 passed；结论为 L2（页面真实发请求/真实文件），解析通用准确率和 OCR 仍 `not_verified`。 |
| P14-P1-06 | `materials.html` 的文件选择 accept 列表包含 `.doc`，但 parser 明确以 `requires_converter` 拒绝；用户会误以为该格式受支持。 | L2 | B1-1、B1-3 | P1 | **C2 已修复**：accept 仅列实际支持的 `.pdf/.txt/.md/.docx/.pptx`，并明确 DOC/PPT/RTF 需先转换；拒绝结果使用安全用户文案。 | `test_p1_4_c2_explainability.py` 4 passed；真实 Chromium 验证 `.doc/.ppt/.rtf/.xml` 0/4 成功且无原始错误码展示。后端 parser contract 未改变。 |
...
| P14-P2-02 | `tasks.html` 只有已知 task id 的读取、取消、重试，没有全局任务列表、筛选和排序。 | L2 | B1-1、B2-2 | P2，C4-3 已关闭 | `GET /api/tasks` 提供 project-scoped、分页、按 status/task_kind/operation_type 筛选的脱敏列表；默认 limit 25，允许 1–100，offset >= 0；稳定按 created_at/id 倒序。页面复用既有详情/取消/重试。 | `test_p1_4_c4_3_task_list.py` 覆盖分页、筛选、边界与 project scope；正式 `/app/tasks.html` 覆盖列表/空/失败/刷新及详情路径。 |
...
| P14-P2-06 | 资料页没有面向用户的批量结果筛选/排序、失败项重试队列和大批量导入进度视图。 | L2 | B1-1、B1-2、B1-5 | P2，C4-5 已关闭（measurement-only） | 已完成有界真实/合成规模与 `/app` 体验测量；当前证据不足以批准新的产品 API、失败队列、进度视图或容量承诺，因此不新增产品能力。后续若达到真实使用摩擦阈值，另立 contract。 | 测量脚本与 browser evidence 记录规模、API/页面耗时、返回上限、失败安全状态；取消未形成成功耗时指标，重复点击沿用既有防抖证据。 |
...
- 正式入口为 `practice.html`，cram session 复用 `practice-session.html`；页面用 `cram_goal_id` URL 参数选择 cram result endpoint。无该参数时普通 practice result 行为保持不变。
- 目标状态只展示并触发后端已允许的 `draft -> active -> completed` 与任意合法状态到 `archived`；完成仍要求至少一个 finished/expired cram session。
- 选题仅显示 `status=ready` 且全部 citation 状态为 `valid` 的题目。无 citation 的 user-created ready 题允许进入；来源已删除/不可用/stale 的引用题不进入选择。
- 目标日期已过是 UI 派生状态，不写入数据库、不伪造新状态：历史目标可见且可归档，但页面不提供激活或新建 session 动作。新目标表单使用当天作为最小日期。
- 所有写操作由共享请求层附加 `Idempotency-Key`，并用页面 busy guard 阻止同一事件循环重复提交；失败后恢复原先业务禁用状态并允许合法重试。
- DOM 只渲染公开 goal/session/result/题目字段；不得渲染 `answer_key`、`answer_json`、路径、SQL、traceback、secret 或原始后端错误。
- 不做自动选题、排程、提醒、Provider 调用、计划/进度/节奏改写，也不实施 C4-2～C4-6。
