...
- UI-ARCHIVE-CONFIRM-887／908：笔记和计划归档增加确认／取消，并在忙碌时阻止再次触发。取消不发送归档请求，确认经既有 API 提交；失败后可再次操作。笔记归档后只读保留并可筛选查看；计划归档后离开默认列表，原有详情链接可回读已归档状态和学习项。
- 本切片仅修改三页前端事件处理；没有改后端 API、schema、持久化或正式数据。两份用户手册及归档测试的确认交互同步更新；涉及的旧测试 fixture 改用本次独立运行目录，避免覆盖共享 fixture。
- 修复前，三个缺陷在 1280×800 与 390×844 均被新专项复现（6 个失败场景）；修复后同一专项 6 passed。报告覆盖两次受控失败后的恢复、鼠标／键盘、准确报告选择、预览、JSON／Markdown 实际下载、刷新和返回；归档覆盖取消零写入、失败恢复、重复点击一次提交、持久化及详情回读。故障注入仅为恢复路径证据。
- 当前聚焦 pytest 为 22 passed：笔记域、计划 API 和前端契约。相关 Chromium 为 71 passed：新专项 6、笔记用户路径 9、笔记契约 10、计划用户路径 5、计划完整契约 24、报告用户路径 14、报告契约 3；重复运行不累加。此次未改后端实现，未重跑完整后端；上一切片的完整后端结果仍为历史运行证据。
- 验收实体由新专项 UI 创建；既有 API-assisted 契约测试单列，不冒充纯 UI 验收。全部为隔离 synthetic／fake 的限定范围 browser-pass；真实 Provider、OCR／ASR 质量和外发未执行。正式三个健康接口只读复核仍不可达，未据此宣称正式服务可用。
- 三项已知失败已修复验证；历史 958 条清单及其替代映射尚未重新普查，不能据本切片宣布全量控件通过或改写历史证据。
