...
> 状态：9B-0 `planned/audit-draft`；9B-1 `planned/contract-frozen`；9B-2、9B-3 `implemented/backend-pass`。
>
> 审计基线：2026-08-30；9B-2 前的稳定实现基线为 schema **v9**、Phase 9A closeout。当前 schema 为 **v10**：9B-2 加入 persistence schema 并通过 migration focused gate；9B-3 已实现并测试共用 repository/domain transaction 基础。9B 的 API、UI、fake-provider generation workflow、export、restore artifact acceptance 与正式用户路径仍未实现。
>
...
- v9 是已完成 Phase 9A 的 schema 基线；v10 `phase9b_material_learning_schema` 加入 note/block/module-link/source-tombstone 与 rhythm persistence schema。9B-3 在 repository/domain 层实现这些 records 的事务操作与显式 source refresh；它不包含 HTTP routes、UI、fake-provider generation workflow、export 或 restore artifact acceptance。
...
> Phase 9B-1 remains `planned/contract-frozen`: S1/S2 entity relations, cadence/timezone/workload rules, note/block/module/citation provenance, state transitions, invariants, fake-provider draft semantics, lifecycle mapping, API/export draft and backup/restore non-repair boundaries are frozen. 9B-2 and 9B-3 are `implemented/backend-pass`: v10 persistence schema plus the shared note/rhythm repository/domain transaction foundation exist and have focused tests. API, UI, fake-provider generation workflow, export, restore artifact acceptance and a formal user path remain unimplemented; this is not Phase 9B completed or real-pass.

Next task: **9B-4 S2 资料笔记与知识模块工作流**. It must reuse the current v10/domain contract and stop for a contract change if any required invariant cannot be enforced safely.
