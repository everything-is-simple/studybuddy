...
- [`phase9b/`](phase9b/)：Phase 9B 总规划 prompt、共用上下文、9B-0 至 9B-9 子任务 prompts、执行顺序和验收门槛；9B-2 至 9B-5 已达到 `implemented/backend-pass`（v10 schema、共用 domain transaction、S2 deterministic fake-provider note workflow、S1 synchronous rhythm workflow），9B-6 至 9B-9 仍为 planned，prompt 目录本身不是实现证据。
