...
- **Phase 9A：学习领域基础与计划核心**：学习目标、知识模块、study plan/item、依赖、进度事件、draft → confirm → active 生命周期，以及 source revision/citation/source_unavailable 关联。不得在此阶段顺带实现 S1–S7 全部体验。充分上下文、子任务切分和逐任务执行 prompts 见 [`PHASE9A_EXECUTION_PROMPTS.md`](PHASE9A_EXECUTION_PROMPTS.md)。该文档只是 planned 执行包，不代表实现完成。
