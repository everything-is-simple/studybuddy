...
- **Phase 9B：资料学习工作流（S1/S2）**：总体仍为 `planned`；9B-0 已完成 `planned/audit-draft`，9B-1 已完成 `planned/contract-frozen`，正式契约见 [`phase9b/PHASE9B_DOMAIN_CONTRACT.md`](phase9b/PHASE9B_DOMAIN_CONTRACT.md)。已建立正式 prompt 包 [`phase9b/`](phase9b/)，包含总规划 prompt、共用上下文、9B-0 至 9B-9 子任务 prompts、执行顺序和 gates。尚未实现 Phase 9B schema、repository、API、UI 或正式用户路径；prompt/contract 不是实现证据。实施时必须先复用已验证的 revision/chunk/retrieval/citation，不得把历史版本的 `KnowledgeModule` 设计直接视为正式实现。
