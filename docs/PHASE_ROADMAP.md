...
**状态：7.1 已完成；7.2 provider/config foundation 已实现并通过 backend tests；7.3–7.5 仍部分实现；hybrid/fallback/Q&A 接入仍未完成。Phase 7 为 partial。**
...
- 已实现：v5 embedding schema migration、非 NULL model revision、status CHECK/updated_at/完整 identity 唯一约束；版本化离线 deterministic fake embedding、独立 registry、embedding 环境配置、f32 little-endian codec、显式 fake indexing、vector cosine API 最小路径和安全 capabilities 扩展。
