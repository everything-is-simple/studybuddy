...
**状态：9A/9B/9C 已在限定范围完成；9D 已部分立项并推进至 9D-7。不得把 Phase 9 作为单一交付承诺。**
...
- **Phase 9D：扩展学习服务（S6/S7，部分立项）**：9D-0 审计结论只立项 deterministic fake/loopback 课堂采集/OCR/ASR、本地脱敏家长报告和 delivery dry-run；真实 OCR/ASR 与真实 SMTP/飞书外发暂不立项。9D-1 已冻结正式契约，9D-2 至 9D-7 均为 `implemented/backend-pass`。9D-7 增加运行配置强制的默认 `off`、allowlisted 无网络 dry-run、live 的配置+授权+显式确认检查后仍固定拒绝、append-only 审计、adapter 前 idempotency replay、显式失败 retry 和 runtime-only secret 边界；不实现 SMTP/飞书 socket/HTTP、scheduler、API 或 UI。focused 9D `27 passed`，完整 backend `352 passed, 2 skipped`。9D-8 至 9D-11 尚未实现，不能宣称 Phase 9D completed。
