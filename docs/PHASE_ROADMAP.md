...
- **Phase 9C：练习与反馈工作流（S3/S4/S5）**：整体仍为 `planned`，其中 9C-0/9C-1 已完成审计与契约冻结，9C-2/9C-3/9C-4/9C-5/9C-6 已达到 `implemented/backend-pass`。v11 migration/schema、共享 domain transaction、S3 PracticeRunner、S4 ErrorFixer 和 S5 ExamCrammer backend 已覆盖 immutable snapshot、server deadline、submit/finish/expire/result、deterministic/pending grading、review/mistake facts、redo、cram goal/session、weak-point、append-only/idempotency、source/privacy 和 rollback；API/UI、生命周期、restore 和最终 acceptance 仍未完成。范围是限时练习、错题改错/人工复核、期末冲刺，详见 [`phase9c/`](phase9c/)。
