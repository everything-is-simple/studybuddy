...
**状态：9A/9B/9C 已在限定范围完成；9D 已部分立项并推进至 9D-3。不得把 Phase 9 作为单一交付承诺。**
...
- **Phase 9D：扩展学习服务（S6/S7，部分立项）**：9D-0 审计结论只立项 deterministic fake/loopback 课堂采集/OCR/ASR、本地脱敏家长报告和 delivery dry-run；真实 OCR/ASR 与真实 SMTP/飞书外发暂不立项。9D-1 已冻结正式契约，9D-2 已实现 v12 migration，9D-3 已实现共享 repository/domain transaction，覆盖 project scope、capture/transcription operation facts、confidence/uncertain、可重算脱敏 report projection、append-only delivery audit、幂等、rollback、secret/raw-response 排除和 material source 降级。9D-2/9D-3 状态为 `implemented/backend-pass`；当前专项 9D domain `6 passed`，相关 9A-9D/migration `49 passed`，完整 backend `331 passed, 2 skipped`。9D-4 至 9D-11 尚未实现，不能宣称 Phase 9D completed。
