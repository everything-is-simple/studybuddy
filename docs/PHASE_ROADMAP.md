...
**状态：9A/9B/9C 已在限定范围完成；9D 已部分立项并推进至 9D-6。不得把 Phase 9 作为单一交付承诺。**
...
- **Phase 9D：扩展学习服务（S6/S7，部分立项）**：9D-0 审计结论只立项 deterministic fake/loopback 课堂采集/OCR/ASR、本地脱敏家长报告和 delivery dry-run；真实 OCR/ASR 与真实 SMTP/飞书外发暂不立项。9D-1 已冻结正式契约，9D-2 已实现 v12 migration，9D-3 已实现共享 repository/domain transaction，9D-4 已实现 S7 deterministic fake/loopback capture/transcription backend，覆盖敏感音频/图片原件上传、hash-derived originals/material lifecycle 绑定、confidence/uncertain、失败/超时/非法输出、幂等 replay/retry、rollback、raw-response 排除和 source lifecycle 安全读路径。9D-2/9D-3/9D-4/9D-5/9D-6 状态为 `implemented/backend-pass`；9D-4/9D-5/9D-6 专项 `14 passed`，相关 migration/governance/domain 回归 `49 passed`，完整 backend `345 passed, 2 skipped`。9D-5 已覆盖同一 capture material 的 confirmed transcript → extraction/revision/chunk/FTS retrieval/citation 接入、draft→edit→confirm/reject、uncertain 保留、用户编辑保护、citation identity 校验和 confirm rollback。9D-6 已覆盖四种报告类型、IANA timezone 半开窗口、只读白名单聚合、source degradation、snapshot/fingerprint replay 和 JSON/Markdown 安全导出。9D-7 至 9D-11 尚未实现，不能宣称 Phase 9D completed。
