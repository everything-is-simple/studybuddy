...
- ARK、硅基流动、Agnes AI-Hub、Sub2API 已建立独立脱敏 capability matrix 和 opt-in 验收命令；API/UI smoke 强制 explicit target 与 runtime provider 匹配，避免将默认 Provider 结果错误归因。ARK、硅基流动和 Agnes 均未发送真实请求并保持 `not_verified`；Agnes 另有独立 child-process launcher/runbook，Sub2API 仍待独立验证。
