...
基础设施的 I1 migration/schema versioning、I2 backup/restore 运维闭环、I3 最小可观察性与 I4 真实环境/容量基线（时间盒）均已完成；StudyBuddy 本地单进程文件材料基础设施 v1 已基本完工。I1 是 AI Phase 4 的硬前置，已满足；I4 的未验证边界已明确记入 v1 运行限制，并作为已知限制持续记录。准确范围见 [`evidence/INFRASTRUCTURE_CLOSEOUT.md`](evidence/INFRASTRUCTURE_CLOSEOUT.md)。当前完整回归基线以 `STATUS.md` 为准：backend 468 passed / 3 skipped、browser 144 passed / 4 skipped；skip 均为 opt-in 真实 smoke。
...
- 已完成：审计 `embeddings`、retrieval run/hit、chunking、migration 和 backup/restore 边界；冻结 embedding identity、status/stale、lexical/vector/hybrid/fallback 语义，以及连续 v5 migration 要求。正式记录见 [`contracts/PHASE7_1_AUDIT_AND_CONTRACT.md`](contracts/PHASE7_1_AUDIT_AND_CONTRACT.md)。
...
**状态：completed（deterministic fake-provider / local Chromium / backup-restore 精确范围）。** v7/v8 migration、Cards/Exercises lifecycle、citation-safe fake-provider draft generation、workspace、append-only review/attempt、deterministic grading、source lifecycle、backup/restore 和完整回归已收口。证据见 [`evidence/PHASE8_ACCEPTANCE_EVIDENCE.md`](evidence/PHASE8_ACCEPTANCE_EVIDENCE.md)。这不表示真实 Provider generation 或全局 Cards/Exercises `real-pass`。
...
**状态：9A/9B/9C 已在限定范围完成；Phase 9D 的 9D-0 部分立项范围已完成 9D-11 scoped closeout。不得把 Phase 9 作为单一交付承诺。**
...
- **Phase 9A：学习领域基础与计划核心**：状态为 `completed`，范围限定为 deterministic fake-provider / local single-process / SQLite / Chromium / backup-restore。9A-0 `planned/audit-draft`、9A-1 `planned/contract-frozen`、9A-2/9A-3/9A-4 `implemented/backend-pass`、9A-5 `browser-pass`、9A-6 `scoped-gates-pass`、9A-7 `restore-gates-pass` 和 9A-8 closeout 均已形成证据。v9 schema、repository/domain transactions、DAG、append-only progress/projection、source identity validation、最小 API、稳定错误、输入边界、本地 Chromium 计划 workspace、delete/restore/purge/re-index source lifecycle 和 backup/verify/restore non-repair 已通过各自 scoped gates。该完成声明不包含 Phase 9B–9D、Phase 9 全部能力或全局 production `real-pass`；最终证据见 [`evidence/PHASE9A_ACCEPTANCE_EVIDENCE.md`](evidence/PHASE9A_ACCEPTANCE_EVIDENCE.md)。
- **Phase 9B：资料学习工作流（S1/S2）**：已在 deterministic fake-provider、单进程 SQLite、本地 Chromium 与 backup/restore 的明确范围内完成。9B-0 为 `planned/audit-draft`，9B-1 为 `planned/contract-frozen`，9B-2 至 9B-6 为 `implemented/backend-pass`，9B-7 为 `browser-pass`，9B-8 为 `scoped-gates-pass`/`restore-gates-pass`，9B-9 已完成 Gate A-I closeout。最终证据见 [`evidence/PHASE9B_ACCEPTANCE_EVIDENCE.md`](evidence/PHASE9B_ACCEPTANCE_EVIDENCE.md)：focused closeout `59 passed`，完整 backend `299 passed, 2 skipped`，相关 Chromium `45 passed, 1 skipped`，默认 real-provider spec `2 skipped`。证据覆盖 v10 migration/rollback、S1 rhythm、S2 note/module/citation、API boundary、Chromium happy/failure/narrow/keyboard/reload、source lifecycle、backup→verify→新空目录 restore non-repair 和隐私边界。该完成声明不包含 Phase 9C/9D、真实 Provider generation、scheduler/worker、人工复核、系统级 screen reader 或全局 production `real-pass`。
- **Phase 9C：练习与反馈工作流（S3/S4/S5）**：已在 deterministic fake-provider、单进程 SQLite、本地 Chromium 与 backup/restore 的明确范围内完成。9C-0 为 `planned/audit-draft`，9C-1 为 `planned/contract-frozen`，9C-2 至 9C-7 为 `implemented/backend-pass`，9C-8 为 `browser-pass`，9C-9 为 `scoped-gates-pass`/`restore-gates-pass`，9C-10 已完成 Gate A-J closeout。最终证据见 [`evidence/PHASE9C_ACCEPTANCE_EVIDENCE.md`](evidence/PHASE9C_ACCEPTANCE_EVIDENCE.md)：focused closeout `59 passed`，完整 backend `320 passed, 2 skipped`，相关 Chromium `12 passed`。证据覆盖 v11 migration/rollback、S3 PracticeRunner、S4 ErrorFixer、S5 ExamCrammer、API boundary、Chromium happy/failure/narrow/keyboard/reload/privacy、source lifecycle、backup→verify→新空目录 restore non-repair 和隐私边界。该完成声明不包含 Phase 9D、真实 Provider generation、scheduler/worker、人工复核、系统级 screen reader 或全局 production `real-pass`。
- **Phase 9D：扩展学习服务（S6/S7，部分立项）**：9D-0 审计结论只立项 deterministic fake/loopback 课堂采集/OCR/ASR、本地脱敏家长报告和 delivery dry-run；真实 OCR/ASR 与真实 SMTP/飞书外发暂不立项。9D-1 已冻结正式契约，9D-2 至 9D-8 均为 `implemented/backend-pass`。9D-7 增加运行配置强制的默认 `off`、allowlisted 无网络 dry-run、live 的配置+授权+显式确认检查后仍固定拒绝、append-only 审计、adapter 前 idempotency replay、显式失败 retry 和 runtime-only secret 边界。9D-8 新增 S7 capture/transcript 全链路 API（create/list/upload/transcribe/edit/confirm/reject）和 S6 report/delivery audit API，转写固定使用 deterministic fake provider，不读取真实凭据；delivery 默认 off，live 三重授权仍返回 `delivery_live_not_approved`。9D-9 新增 desktop/narrow/keyboard/reload/failure/privacy Chromium workspace，覆盖采集、转写、报告预览/导出、默认关闭交付和审计；focused API `4 passed`、Chromium `4 passed`、相关 UI 回归 `15 passed`。9D-10 新增 source lifecycle（软删/硬删降级）、backup/restore 门禁（verify→restore non-repair、不触发 AI/OCR/ASR/报告/交付），`test_phase9d_backup_restore.py` 4 passed。9D-11 完成 Gate A-L 的部分立项范围收口，focused 9D backend `35 passed`，相关 Phase 8/9 UI regression `22 passed`，完整 backend `361 passed, 2 skipped`，完整 Chromium `52 passed, 3 skipped`（仅 opt-in real-provider browser paths skipped）；证据见 `PHASE9D_ACCEPTANCE_EVIDENCE.md`。准确声明限于 9D-0 批准的 deterministic fake/loopback、本地 SQLite、Chromium、backup/restore 和 dry-run 范围。

Phase 9A、Phase 9B 与 Phase 9C 已分别在明确范围内完成；Phase 9D 的 9D-0 部分立项范围已按独立 gate 完成 scoped closeout，未立项的真实 OCR/ASR 与真实外发不计入完成声明。**Phase 10 已完成 10-0 至 10-9（Gates A-J）：StudyBuddy 已在 local single-process / single-instance / SQLite / local-disk v1 的明确范围内完成生产化和上线收口。** runner 默认不自动启动，只有 approved `embedding_index` provider-backed 阶段可经显式 API/CLI 执行；safe observability/readiness/diagnostics、explicit backup/restore/migration operations、local runtime release、boundary evidence 和隔离 release-candidate drill 均已通过。Phase 9 全部完成也不等于全局生产级 `real-pass`。

### Phase 10：本地生产化、后台任务与上线收口

**状态：completed（限定 local single-process / single-instance / SQLite / local-disk v1）。10-0 至 10-8 已分别通过 Gates A-I；10-9 已在隔离 data root 完成 release candidate 端到端演练、完整 backend/相关 Chromium 回归、evidence 和文档收口，Gate J 已通过。10-6 提供 explicit upgrade preflight、verified backup rotation、historical rollback backup verification、restore drill 与 stop/quarantine policy；10-7 提供 loopback/单进程 serve、data-root lock、Windows start/health/stop 和 safe configuration defaults。真实断电、ACL、磁盘满、网络文件系统、universal installer 仍为 `not_verified`。runner 仍不自动启动；Q&A、generation、OCR/ASR、report/delivery 未接入。此 completed 结论不将多用户互联网服务、共享 data root 多进程或全局 production `real-pass` 作为隐含支持。**

10-0 审计产物见 [`archive/PHASE10_AUDIT_AND_SCOPE.md`](archive/PHASE10_AUDIT_AND_SCOPE.md)，Gate A 已通过；10-1 operation/task 契约见 [`contracts/PHASE10_OPERATION_TASK_CONTRACT.md`](contracts/PHASE10_OPERATION_TASK_CONTRACT.md)，Gate B 已通过；10-2 已新增 v13 `phase10_operation_task_schema` 并通过 migration/backup/restore Gate C；10-3 已实现 `task_runner.py`、cooperative cancel、lease/stale/restart/shutdown recovery 且不自动启动，Gate D 已通过；10-4 已将 approved `embedding_index` provider-backed 阶段接入显式 API/CLI runner，Gate E 已通过；10-5 已完成 task-aware safe structured events、process metrics、readiness/degraded 和 explicit read-only diagnostics，Gate F 已通过；10-6 已完成 non-mutating upgrade preflight、verified rotation、history-compatible rollback backup verification、restore drill 和 stop/quarantine policy，Gate G 已通过；10-7 已完成 local runtime release，Gate H 已通过；10-8 已完成容量/性能/长时 boundary evidence，Gate I 已通过；10-9 已完成 Gate J release candidate drill、完整回归与文档收口。证据见 [`evidence/PHASE10_TASK_INTEGRATION_EVIDENCE.md`](evidence/PHASE10_TASK_INTEGRATION_EVIDENCE.md)、[`evidence/PHASE10_OBSERVABILITY_READINESS_EVIDENCE.md`](evidence/PHASE10_OBSERVABILITY_READINESS_EVIDENCE.md)、[`evidence/PHASE10_OPERATIONS_EVIDENCE.md`](evidence/PHASE10_OPERATIONS_EVIDENCE.md)、[`evidence/PHASE10_RELEASE_RUNTIME_EVIDENCE.md`](evidence/PHASE10_RELEASE_RUNTIME_EVIDENCE.md)、[`evidence/PHASE10_BOUNDARY_EVIDENCE.md`](evidence/PHASE10_BOUNDARY_EVIDENCE.md) 和 [`evidence/PHASE10_RELEASE_CANDIDATE_EVIDENCE.md`](evidence/PHASE10_RELEASE_CANDIDATE_EVIDENCE.md)。Phase 10 共 10 步（10-0 至 10-9），必须一次执行一个、逐步测试、逐步提交和逐步更新证据：
