...
- **Phase 9C：练习与反馈工作流（S3/S4/S5）**：当前为 `planned`。范围是限时练习、错题改错/人工复核、期末冲刺；依赖 Phase 8 的 exercise/card 生命周期、append-only attempt 历史和确定性评分边界。已完成总体拆分和备用 prompt 包，见 [`phase9c/`](phase9c/)；这不是实现证据，S3/S4/S5 必须分别实现并验收。
