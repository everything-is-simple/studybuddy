...
- **Phase 9C：练习与反馈工作流（S3/S4/S5）**：整体仍为 `planned`，其中 9C-0/9C-1 已完成审计与契约冻结，9C-2/9C-3/9C-4/9C-5 已达到 `implemented/backend-pass`。v11 migration/schema、共享 domain transaction、S3 PracticeRunner 和 S4 ErrorFixer backend 已覆盖 immutable snapshot、server deadline、submit/finish/expire/result、deterministic/pending grading、review decisions、mistake facts、redo、weak-point、append-only/idempotency、source/privacy 和 rollback；S5 独立 workflow、API/UI、生命周期、restore 和最终 acceptance 仍未完成。范围是限时练习、错题改错/人工复核、期末冲刺，详见 [`phase9c/`](phase9c/)。
- **Phase 9D：扩展学习服务（S6/S7，条件性范围）**：家长报告、课堂采集/OCR/ASR。只有在用户需求、隐私边界、真实组件证据和运维成本明确后才立项；不因历史版本存在就自动纳入正式范围。

Phase 9A 与 Phase 9B 已分别在明确范围内完成；Phase 9C–9D 仍是独立的 planned work。每个后续子阶段完成后才能进入下一个；Phase 9 全部完成也不等于全局生产级 `real-pass`。
