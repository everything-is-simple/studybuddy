...
**状态：completed（deterministic fake-provider / local Chromium / backup-restore 精确范围）。** v7/v8 migration、Cards/Exercises lifecycle、citation-safe fake-provider draft generation、workspace、append-only review/attempt、deterministic grading、source lifecycle、backup/restore 和完整回归已收口。证据见 [`prompts/PHASE8_ACCEPTANCE_EVIDENCE.md`](prompts/PHASE8_ACCEPTANCE_EVIDENCE.md)。这不表示真实 Provider generation 或全局 Cards/Exercises `real-pass`。
...
- **Phase 9A：学习领域基础与计划核心**：状态为 `completed`，范围限定为 deterministic fake-provider / local single-process / SQLite / Chromium / backup-restore。9A-0 `planned/audit-draft`、9A-1 `planned/contract-frozen`、9A-2/9A-3/9A-4 `implemented/backend-pass`、9A-5 `browser-pass`、9A-6 `scoped-gates-pass`、9A-7 `restore-gates-pass` 和 9A-8 closeout 均已形成证据。v9 schema、repository/domain transactions、DAG、append-only progress/projection、source identity validation、最小 API、稳定错误、输入边界、本地 Chromium 计划 workspace、delete/restore/purge/re-index source lifecycle 和 backup/verify/restore non-repair 已通过各自 scoped gates。该完成声明不包含 Phase 9B–9D、Phase 9 全部能力或全局 production `real-pass`；最终证据见 [`prompts/PHASE9A_ACCEPTANCE_EVIDENCE.md`](prompts/PHASE9A_ACCEPTANCE_EVIDENCE.md)。充分上下文、总规划、子任务切分和逐任务执行 prompts 见 [`prompts/phase9a/`](prompts/phase9a/)。
- **Phase 9B：资料学习工作流（S1/S2）**：已在 deterministic fake-provider、单进程 SQLite、本地 Chromium 与 backup/restore 的明确范围内完成。充分上下文见 [`prompts/phase9b/`](prompts/phase9b/)。9B-0 为 `planned/audit-draft`，9B-1 为 `planned/contract-frozen`，9B-2 至 9B-6 为 `implemented/backend-pass`，9B-7 为 `browser-pass`，9B-8 为 `scoped-gates-pass`/`restore-gates-pass`，9B-9 已完成 Gate A-I closeout。最终证据见 [`prompts/PHASE9B_ACCEPTANCE_EVIDENCE.md`](prompts/PHASE9B_ACCEPTANCE_EVIDENCE.md)：focused closeout `59 passed`，完整 backend `299 passed, 2 skipped`，相关 Chromium `45 passed, 1 skipped`，默认 real-provider spec `2 skipped`。证据覆盖 v10 migration/rollback、S1 rhythm、S2 note/module/citation、API boundary、Chromium happy/failure/narrow/keyboard/reload、source lifecycle、backup→verify→新空目录 restore non-repair 和隐私边界。该完成声明不包含 Phase 9C/9D、真实 Provider generation、scheduler/worker、人工复核、系统级 screen reader 或全局 production `real-pass`。
- **Phase 9C：练习与反馈工作流（S3/S4/S5）**：已在 deterministic fake-provider、单进程 SQLite、本地 Chromium 与 backup/restore 的明确范围内完成。充分上下文见 [`prompts/phase9c/`](prompts/phase9c/)。9C-0 为 `planned/audit-draft`，9C-1 为 `planned/contract-frozen`，9C-2 至 9C-7 为 `implemented/backend-pass`，9C-8 为 `browser-pass`，9C-9 为 `scoped-gates-pass`/`restore-gates-pass`，9C-10 已完成 Gate A-J closeout。最终证据见 [`prompts/PHASE9C_ACCEPTANCE_EVIDENCE.md`](prompts/PHASE9C_ACCEPTANCE_EVIDENCE.md)：focused closeout `59 passed`，完整 backend `320 passed, 2 skipped`，相关 Chromium `12 passed`。证据覆盖 v11 migration/rollback、S3 PracticeRunner、S4 ErrorFixer、S5 ExamCrammer、API boundary、Chromium happy/failure/narrow/keyboard/reload/privacy、source lifecycle、backup→verify→新空目录 restore non-repair 和隐私边界。该完成声明不包含 Phase 9D、真实 Provider generation、scheduler/worker、人工复核、系统级 screen reader 或全局 production `real-pass`。
...
Phase 10 的总体规划、共用上下文、逐任务 prompts、执行顺序和 Gate A-J 已保存于 [`prompts/phase10/`](prompts/phase10/)。10-0 审计产物见 [`prompts/phase10/PHASE10_AUDIT_AND_SCOPE.md`](prompts/phase10/PHASE10_AUDIT_AND_SCOPE.md)，Gate A 已通过；10-1 operation/task 契约见 [`prompts/phase10/PHASE10_OPERATION_TASK_CONTRACT.md`](prompts/phase10/PHASE10_OPERATION_TASK_CONTRACT.md)，Gate B 已通过；10-2 已新增 v13 `phase10_operation_task_schema` 并通过 migration/backup/restore Gate C；10-3 已实现 `task_runner.py`、cooperative cancel、lease/stale/restart/shutdown recovery 且不自动启动，Gate D 已通过；10-4 已将 approved `embedding_index` provider-backed 阶段接入显式 API/CLI runner，Gate E 已通过；10-5 已完成 task-aware safe structured events、process metrics、readiness/degraded 和 explicit read-only diagnostics，Gate F 已通过；10-6 已完成 non-mutating upgrade preflight、verified rotation、history-compatible rollback backup verification、restore drill 和 stop/quarantine policy，Gate G 已通过；10-7 已完成 local runtime release，Gate H 已通过；10-8 已完成容量/性能/长时 boundary evidence，Gate I 已通过；10-9 已完成 Gate J release candidate drill、完整回归与文档收口。证据见 [`prompts/phase10/PHASE10_TASK_INTEGRATION_EVIDENCE.md`](prompts/phase10/PHASE10_TASK_INTEGRATION_EVIDENCE.md)、[`prompts/phase10/PHASE10_OBSERVABILITY_READINESS_EVIDENCE.md`](prompts/phase10/PHASE10_OBSERVABILITY_READINESS_EVIDENCE.md)、[`prompts/phase10/PHASE10_OPERATIONS_EVIDENCE.md`](prompts/phase10/PHASE10_OPERATIONS_EVIDENCE.md)、[`prompts/phase10/PHASE10_RELEASE_RUNTIME_EVIDENCE.md`](prompts/phase10/PHASE10_RELEASE_RUNTIME_EVIDENCE.md)、[`prompts/phase10/PHASE10_BOUNDARY_EVIDENCE.md`](prompts/phase10/PHASE10_BOUNDARY_EVIDENCE.md) 和 [`prompts/phase10/PHASE10_RELEASE_CANDIDATE_EVIDENCE.md`](prompts/phase10/PHASE10_RELEASE_CANDIDATE_EVIDENCE.md)。Phase 10 共 10 步（10-0 至 10-9），必须一次执行一个、逐步测试、逐步提交和逐步更新证据：
