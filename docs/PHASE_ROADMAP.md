...
- **Phase 9C：练习与反馈工作流（S3/S4/S5）**：整体仍为 `planned`，其中 9C-0/9C-1 已完成审计与契约冻结，9C-2 已达到 `implemented/backend-pass`。v11 migration/schema 已通过 new DB、v10 upgrade、幂等、rollback、约束、history/user_version 和 backup version gates；S3/S4/S5 repository/domain、API/UI、生命周期、restore 和最终 acceptance 仍未完成。范围是限时练习、错题改错/人工复核、期末冲刺，详见 [`phase9c/`](phase9c/)。
