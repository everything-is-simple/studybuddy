...
- **Phase 9B：资料学习工作流（S1/S2）**：总体仍为 `planned`；9B-0 已完成 `planned/audit-draft`，9B-1 已完成 `planned/contract-frozen`，9B-2 至 9B-5 已完成 `implemented/backend-pass`。正式契约见 [`phase9b/PHASE9B_DOMAIN_CONTRACT.md`](phase9b/PHASE9B_DOMAIN_CONTRACT.md)：连续 v10 `phase9b_material_learning_schema` 已增加 note/block/module-link/source-tombstone 与 rhythm persistence schema，并通过 new-db、v9-upgrade、rollback、history/user_version 和 backup schema-version focused tests。9B-3 在 `backend/app/repository.py` 实现共用 domain transaction；9B-4 增加单材料 deterministic fake-provider note draft；9B-5 完成显式同步的 daily/weekly IANA-timezone rhythm settings、受保护的 local-date allocation、确定性 timeline/load/progress/source-warning summary 和 rollback/lock retry backend tests；9B-6 在此基础上完成 S1/S2 最小安全 FastAPI，包括 rhythm、note/block/module/source link、fake-provider draft generation、状态转换、source refresh 和 bounded JSON/Markdown export，复用 server project scope 与稳定错误 contract。9B-7 已在既有单页 workspace 中完成 S1/S2 Chromium scoped gate：S1 的 rhythm settings/allocation/summary/progress/reload，以及 S2 的 user/AI cited draft、module organization、edit/transition/source refresh/export，并覆盖 provider-not-configured、malformed/network failure、unavailable citation、duplicate click、390x844 与 keyboard。9B-8 已通过 scoped lifecycle 与 restore gates：delete/restore/purge/new revision 后 note source link 的 valid/stale/source_deleted/source_unavailable、confirmed/user-edited/archived note/module、rhythm/allocation 与 completed progress history 均保留；backup→verify→新空目录 restore 保留 v10 state，restore acceptance 只读检查 notes/rhythm 且不 provider/index/refresh/repair。9B-9 的完整收口仍未实现；这些局部 gates 不是完整功能证据。实施时必须先复用已验证的 revision/chunk/retrieval/citation，不得把历史版本的 `KnowledgeModule` 设计直接视为正式实现。
