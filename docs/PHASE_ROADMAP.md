...
Phase 9A、Phase 9B 与 Phase 9C 已分别在明确范围内完成；Phase 9D 的 9D-0 部分立项范围已按独立 gate 完成 scoped closeout，未立项的真实 OCR/ASR 与真实外发不计入完成声明。Phase 10 已完成 10-0 至 10-8（Gates A-I）；runner 默认不自动启动，只有 approved `embedding_index` provider-backed 阶段可经显式 API/CLI 执行；safe observability/readiness/diagnostics、explicit backup/restore/migration operations、local runtime release 和 scoped boundary evidence 已实现。Phase 9 全部完成也不等于全局生产级 `real-pass`。

### Phase 10：本地生产化、后台任务与上线收口

**状态：规划已完成；10-0 已完成现状审计与范围冻结（`planned/audit-draft`，Gate A 通过），10-1 已完成 operation/task 契约冻结（`planned/contract-frozen`，Gate B 通过），10-2 至 10-7 已分别完成 task schema、explicit runner/recovery、approved `embedding_index` 接入、safe observability/readiness/diagnostics、backup/restore/migration operations 与 local runtime release（均为 `implemented/backend-pass`；Gates C-H 通过）；10-8 已完成 scoped boundary evidence（Gate I）。10-6 提供 explicit upgrade preflight、verified backup rotation、historical rollback backup verification、restore drill 与 stop/quarantine policy；10-7 提供 loopback/单进程 serve、data-root lock、Windows start/health/stop 和 safe configuration defaults。真实断电、ACL、磁盘满、网络文件系统、universal installer 仍为 `not_verified`。runner 仍不自动启动；Q&A、generation、OCR/ASR、report/delivery 未接入。推荐目标是 local single-process / single-instance / SQLite / local-disk v1 上线，不将多用户互联网服务作为隐含目标。**

Phase 10 的总体规划、共用上下文、逐任务 prompts、执行顺序和 Gate A-J 已保存于 [`prompts/phase10/`](prompts/phase10/)。10-0 审计产物见 [`prompts/phase10/PHASE10_AUDIT_AND_SCOPE.md`](prompts/phase10/PHASE10_AUDIT_AND_SCOPE.md)，Gate A 已通过；10-1 operation/task 契约见 [`prompts/phase10/PHASE10_OPERATION_TASK_CONTRACT.md`](prompts/phase10/PHASE10_OPERATION_TASK_CONTRACT.md)，Gate B 已通过；10-2 已新增 v13 `phase10_operation_task_schema` 并通过 migration/backup/restore Gate C；10-3 已实现 `task_runner.py`、cooperative cancel、lease/stale/restart/shutdown recovery 且不自动启动，Gate D 已通过；10-4 已将 approved `embedding_index` provider-backed 阶段接入显式 API/CLI runner，Gate E 已通过；10-5 已完成 task-aware safe structured events、process metrics、readiness/degraded 和 explicit read-only diagnostics，Gate F 已通过；10-6 已完成 non-mutating upgrade preflight、verified rotation、history-compatible rollback backup verification、restore drill 和 stop/quarantine policy，Gate G 已通过；10-7 已完成 local runtime release，Gate H 已通过；10-8 已完成容量/性能/长时 boundary evidence，Gate I 已通过。证据见 [`prompts/phase10/PHASE10_TASK_INTEGRATION_EVIDENCE.md`](prompts/phase10/PHASE10_TASK_INTEGRATION_EVIDENCE.md)、[`prompts/phase10/PHASE10_OBSERVABILITY_READINESS_EVIDENCE.md`](prompts/phase10/PHASE10_OBSERVABILITY_READINESS_EVIDENCE.md)、[`prompts/phase10/PHASE10_OPERATIONS_EVIDENCE.md`](prompts/phase10/PHASE10_OPERATIONS_EVIDENCE.md)、[`prompts/phase10/PHASE10_RELEASE_RUNTIME_EVIDENCE.md`](prompts/phase10/PHASE10_RELEASE_RUNTIME_EVIDENCE.md) 和 [`prompts/phase10/PHASE10_BOUNDARY_EVIDENCE.md`](prompts/phase10/PHASE10_BOUNDARY_EVIDENCE.md)。Phase 10 共 10 步（10-0 至 10-9），必须一次执行一个、逐步测试、逐步提交和逐步更新证据：
