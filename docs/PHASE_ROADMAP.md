...
- **Phase 9B：资料学习工作流（S1/S2）**：总体仍为 `planned`；9B-0 已完成 `planned/audit-draft`，9B-1 已完成 `planned/contract-frozen`，9B-2 已完成 `implemented/backend-pass`。正式契约见 [`phase9b/PHASE9B_DOMAIN_CONTRACT.md`](phase9b/PHASE9B_DOMAIN_CONTRACT.md)：连续 v10 `phase9b_material_learning_schema` 已增加 note/block/module-link/source-tombstone 与 rhythm persistence schema，并通过 new-db、v9-upgrade、rollback、history/user_version 和 backup schema-version focused tests。尚未实现 Phase 9B repository/domain、API、UI、source lifecycle、export、restore artifact acceptance 或正式用户路径；schema/prompt/contract 不是完整功能证据。实施时必须先复用已验证的 revision/chunk/retrieval/citation，不得把历史版本的 `KnowledgeModule` 设计直接视为正式实现。
