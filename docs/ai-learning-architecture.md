...
状态：material revision、deterministic chunks、lexical/vector/hybrid retrieval、context/citation、Q&A、Cards/Exercises，以及 Phase 9A/9B/9C 已按各自证据实现并验收；Phase 9D 已在部分立项范围完成 v12 schema、共享 domain backend、S7 deterministic fake/loopback capture/transcription backend、S7→S2 confirmed transcript ingestion backend 和 S6 report aggregation/redaction backend。schema 由 Phase 9A 的 v9、Phase 9B 的 v10、Phase 9C 的 v11 连续演进到当前 v12；各阶段完成声明分别限于对应 acceptance 文档记录的 deterministic fake-provider、本地单进程 SQLite、Chromium 与 backup/restore 范围，证据见 [`PHASE9A_ACCEPTANCE_EVIDENCE.md`](PHASE9A_ACCEPTANCE_EVIDENCE.md)、[`PHASE9B_ACCEPTANCE_EVIDENCE.md`](PHASE9B_ACCEPTANCE_EVIDENCE.md)、[`PHASE9C_ACCEPTANCE_EVIDENCE.md`](PHASE9C_ACCEPTANCE_EVIDENCE.md)。S6 report 仅支持显式 backend 生成的 daily/weekly/monthly/exam_alert 安全聚合、snapshot/fingerprint、JSON/Markdown 导出和 source degradation；Phase 9D 当前不包含真实 OCR/ASR、真实交付、API/UI、完整 source lifecycle/restore gates 或全局 production `real-pass`；S2 ingestion 仅支持显式 draft edit/confirm/reject 后的 backend 接入。
