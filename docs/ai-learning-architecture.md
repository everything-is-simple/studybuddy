...
状态：material revision、deterministic chunks、lexical/vector/hybrid retrieval、context/citation、Q&A、Cards/Exercises，以及 Phase 9A/9B/9C 已按各自证据实现并验收。schema 由 Phase 9A 的 v9、Phase 9B 的 v10 连续演进到当前 Phase 9C 的 v11；Phase 8 和 Phase 9A/9B/9C 的完成声明分别限于各自 acceptance 文档记录的 deterministic fake-provider、本地单进程 SQLite、Chromium 与 backup/restore 范围，见 [`PHASE8_ACCEPTANCE_EVIDENCE.md`](PHASE8_ACCEPTANCE_EVIDENCE.md)、[`PHASE9A_ACCEPTANCE_EVIDENCE.md`](PHASE9A_ACCEPTANCE_EVIDENCE.md)、[`PHASE9B_ACCEPTANCE_EVIDENCE.md`](PHASE9B_ACCEPTANCE_EVIDENCE.md) 和 [`PHASE9C_ACCEPTANCE_EVIDENCE.md`](PHASE9C_ACCEPTANCE_EVIDENCE.md)。Phase 9C 已覆盖 S3 限时练习、S4 错题改错与复核、S5 期末冲刺的 v11 persistence、domain/API/UI、source lifecycle 和 restore non-repair。真实 Provider generation、scheduler/worker、人工简答复核、Phase 9D、多用户和全局 production `real-pass` 仍未实现或未验证。
