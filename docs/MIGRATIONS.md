...
- v13 adds Phase 10 task envelopes (`operation_tasks`) and append-only task-attempt audit (`operation_task_attempts`). It preserves existing `ai_operations` and does not backfill historical operations, create a runner, execute tasks, alter legacy synchronous APIs, or persist raw content, secrets, paths, raw Provider payloads, answer keys, or submitted answers. Composite project-scoped FKs, status/progress/retry checks, one-task-per-operation and at-most-one-running-attempt indexes provide structural protection; state transitions, progress monotonicity, lease compare-and-set, cancellation and retry policy remain repository/runner behavior.
