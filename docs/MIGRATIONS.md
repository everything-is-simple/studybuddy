...
- v12 adds the Phase 9D capture-session, transcript draft/segment, report snapshot, delivery-attempt, and capture-linked operation persistence schema. 9D-3 shared domain behavior, 9D-4 deterministic fake/loopback capture/transcription backend and 9D-5 explicit confirmed transcript ingestion into the existing S2 material/revision/chunk/retrieval/citation path now sit above this migration; report aggregation, delivery behavior, API/UI and restore acceptance remain outside the completed migration/domain subset.
