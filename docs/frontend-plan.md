...
1. **默认 `/app` 集成**：P1-1 已迁移材料详情索引、Q&A citation detail/body location：材料详情展示解析正文并可建立 AI 索引；问答历史按需加载线程并将有效引用导航到对应材料正文高亮位置，删除或不可用来源明确标记。后续继续迁移计划/笔记、Cards/Exercises 和其余 `legacy_only` 学习写操作；`/legacy` 只保留兼容回退。
