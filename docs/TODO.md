...
> 更新：2026-09-05（执行方向修订：证据梯子 → 可用优先；P2-USE 五个切片已全部完成；P2-FE-2 场景 2 合同已冻结，P2-FE-3 已完成材料导入、回收站/导出、QA 核心链路、线程工作区与错误映射、P6-C 跨页连接五批正式证据迁移，以及场景 3 三批练习流程正式证据迁移；活动主线继续为 **P2-FE（前后端场景对齐）**）
> 当前基线：本地单进程文件材料管理基础系统已完成 local v1 上线收口，正式 schema 为 v14；当前后端回归为 **618 passed, 1 failed, 4 errors, 3 skipped**（失败/错误集中于既有 P14 revision-fingerprint migration 测试），完整 Chromium 为 **206 passed, 4 skipped**（210 tests）。skip 均为 opt-in 真实 smoke。此轮已修正测试服务端口隔离、Phase 9D deterministic fixture 的显式 fake 配置和 review 失败→重试合同；正式入口统一到 `today.html`；**Plans → Today → Progress 链路已实现**：`today.html` 只显示 active plan 当天 allocation，`plan-detail.html` 提供 progress 记录按钮，`plans.html` 提供详情入口，跨页测试 `3 passed`（含 Today 失败注入→重试恢复）。**P2-FE-1 前端事实盘点已完成**；P2-FE-2 已完成“计划 → 今天 → 进度”完整模板与 27 个 `unreached` path key 定性，并同步交付进度历史和 Today 三类可操作空态，详见 [`frontend-inventory-report.md`](frontend-inventory-report.md) 与 [`contracts/frontend-scenario-contract.md`](contracts/frontend-scenario-contract.md)。整体阶段性完成度约 **65%**。前端 A3/A4 仅代表已验收的静态页面与限定行为；Neutral Modern 已在已验收静态页面范围完成，但不代表完整产品化页面架构、deferred capability 或全局 real-pass。Phase 9D 的 9D-0 部分立项范围已完成 9D-11 scoped closeout，完整状态见 [`STATUS.md`](STATUS.md)、[`evidence/PHASE9D_ACCEPTANCE_EVIDENCE.md`](evidence/PHASE9D_ACCEPTANCE_EVIDENCE.md) 与 [`evidence/PHASE10_RELEASE_CANDIDATE_EVIDENCE.md`](evidence/PHASE10_RELEASE_CANDIDATE_EVIDENCE.md)。
>
...
- [x] 9A-0：完成现状审计、9A non-goals、9B/9C/9D 边界和风险冻结；初稿见 [`contracts/PHASE9A_DOMAIN_CONTRACT.md`](contracts/PHASE9A_DOMAIN_CONTRACT.md)。状态为 `planned/audit-draft`，不代表领域能力实现。
- [x] 9A-1：冻结 learning goal、knowledge module、study plan/item、dependency、progress event、source link 的正式领域契约和状态机；正式契约见 [`contracts/PHASE9A_DOMAIN_CONTRACT.md`](contracts/PHASE9A_DOMAIN_CONTRACT.md)。状态为 `planned/contract-frozen`，不代表 schema/API/UI 实现。
- [x] 9A-2：通过连续 v9 migration 增加计划、目标、模块、依赖、进度事件和 source link schema，并完成 new-db/v8-upgrade/rollback/幂等和 backup/restore schema-history 测试；见 [`contracts/PHASE9A_DOMAIN_CONTRACT.md`](contracts/PHASE9A_DOMAIN_CONTRACT.md)。状态为 `implemented/backend-pass`，repository/domain 及其后续 API/UI 另由 9A-3 至 9A-5 验收。
...
>
> 当前子任务状态：9B-0 为 `planned/audit-draft`，9B-1 为 `planned/contract-frozen`，9B-2 至 9B-6 为 `implemented/backend-pass`，9B-7 为 `browser-pass`，9B-8 为 `scoped-gates-pass`/`restore-gates-pass`，9B-9 已完成限定范围内 `completed` closeout；审计、正式契约、schema 和最终证据见 [`contracts/PHASE9B_DOMAIN_CONTRACT.md`](contracts/PHASE9B_DOMAIN_CONTRACT.md) 与 [`evidence/PHASE9B_ACCEPTANCE_EVIDENCE.md`](evidence/PHASE9B_ACCEPTANCE_EVIDENCE.md)。

- [x] 9B-0：完成现状审计、S1/S2 范围冻结和风险记录；状态为 `planned/audit-draft`，产出见 [`contracts/PHASE9B_DOMAIN_CONTRACT.md`](contracts/PHASE9B_DOMAIN_CONTRACT.md)。
...
- [x] 9B-9：完成 Gate A-I 全量验收、脱敏 evidence 与最终文档同步。focused closeout `59 passed`；完整 backend `299 passed, 2 skipped`；相关非真实 Provider Chromium `45 passed, 1 skipped`；默认 real-provider spec `2 skipped`。新增 [`evidence/PHASE9B_ACCEPTANCE_EVIDENCE.md`](evidence/PHASE9B_ACCEPTANCE_EVIDENCE.md)，同步 STATUS/README/ROADMAP/PROJECT_PROGRESS/INDEX/architecture。状态为限定范围内 `completed`，不代表 Phase 9C/9D、真实 Provider generation、scheduler/worker、人工复核或全局 production `real-pass`。
...
- [x] 9C-10：完成 Gate A-J、全量回归、脱敏 evidence、STATUS/TODO/ROADMAP/PROJECT_PROGRESS/INDEX 文档收口；证据见 [`evidence/PHASE9C_ACCEPTANCE_EVIDENCE.md`](evidence/PHASE9C_ACCEPTANCE_EVIDENCE.md)。
...
- [x] 9D-11：完成 Gate A-L 的部分立项范围收口、完整 backend/UI 回归、脱敏 evidence 和 STATUS/TODO/ROADMAP/PROJECT_PROGRESS/INDEX/README 同步；证据见 [`evidence/PHASE9D_ACCEPTANCE_EVIDENCE.md`](evidence/PHASE9D_ACCEPTANCE_EVIDENCE.md)。真实 OCR/ASR provider 与真实 SMTP/飞书外发仍明确不在本次立项范围。
...
- [x] 10-1：operation/task 正式契约与状态机；Gate B 已通过（`planned/contract-frozen`）。证据：[`contracts/PHASE10_OPERATION_TASK_CONTRACT.md`](contracts/PHASE10_OPERATION_TASK_CONTRACT.md)。
...
> 权威执行路线：[`ROADMAP_CAPABILITIES.md`](ROADMAP_CAPABILITIES.md)。此 P1 不改变 Phase 10 local v1 的完成结论。`/app` 是默认正式入口，`/legacy` 仅为兼容回退；先完成文档一致性和遗留用户操作集成，再扩展 B1-B4 验证范围。桌面化明确暂缓，直至所有现有 Web 功能完成集成并正常验证。
...
- [ ] P2-FE-3：按合同实施（第三阶段）。**进行中：场景 2 五批已完成；场景 3 三批已完成。**
  ...
  本轮回归：focused browser `13 passed`（场景 3 三批共 13 test）；完整 Chromium `206 passed, 4 skipped`（210 tests）。Backend 回归为 `618 passed, 1 failed, 4 errors, 3 skipped`，失败/错误集中于既有 P14 revision-fingerprint migration 测试，非本批前端改动。
  场景 2 五批总计 15 test，场景 3 三批总计 13 test，状态均为 `implemented / scoped-browser-pass`。内联脚本模块化仍按计划后置。
  尚未完成：
