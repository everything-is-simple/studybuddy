...
- [ ] B2：真实 OCR 已选 PaddleOCR 主路径、RapidOCR ONNX 回退（Tesseract 仅兼容后备）；完成预置本地模型、合成中英/空白/损坏/超大图片、无隐式网络、超时/清理/脱敏 Composer smoke -> Integration -> Formal `ImageOcrProvider`；结果必须 draft-first 并需用户确认。
