...
- [ ] B0（进行中）：已建立 `H:\studybuddy-composer\B0-COMPONENT-GOVERNANCE.md`、四类能力候选 catalog、统一 component-card 字段和安全/忽略规则。当前 C0 已选定 `H:\WhisperCli`/whisper.cpp `large-v3-turbo`（唯一 ASR runtime）、PaddleOCR 3.7.0 主 OCR、RapidOCR ONNX 1.4.4 回退、edge-tts 7.2.8 免费在线 TTS 候选，以及 formal-pptx + MarkItDown + 图片页 OCR 的 PPTX 三层路径；ASR 选定候选已通过独立 C1 smoke；OCR、报告、外发候选的 C1 smoke、所有候选 Integration 与 Formal 仍未通过，不能进入正式系统。ASR C1 evidence：`H:\studybuddy-composer\results\asr-whisper-cpp\c1-smoke.json`；验证命令：`C:\miniconda\py310\python.exe H:\studybuddy-composer\scripts\validate-b0-catalog.py`。
