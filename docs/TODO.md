...
> 更新：2026-08-30（Phase 10-9、A3/A4、A3-FC-3-2、A3-PAGES、A3-VISUAL 与 B0 governance scaffold、ASR C1 smoke evidence 已同步）
...
- [ ] B2：真实 OCR 已选 PaddleOCR 主路径、RapidOCR ONNX 回退（Tesseract 仅兼容后备）；PaddleOCR 与 RapidOCR ONNX 均完成 Composer C1 smoke（精确为 PaddleOCR 3.7.0/PaddlePaddle 3.3.1 + PP-OCRv5_server_det/rec、RapidOCR 1.4.4 bundled ONNX、Windows/Python 3.10，均 `12/12` 通过），证据：`H:\studybuddy-composer\results\ocr-paddleocr\c1-smoke.json`、`H:\studybuddy-composer\results\ocr-rapidocr\c1-smoke.json`。B2 的 PaddleOCR C2 Integration 已在隔离 Integration 目录通过（原件 hash/storage、draft-first confidence/uncertain、确认后同 material revision、source lifecycle、rollback、backup/restore non-repair），证据：`H:\studybuddy-integration\results\ocr-paddleocr-c2\integration.json`；C3-C6 Formal `ImageOcrProvider` 仍待执行。结果必须 draft-first 并需用户确认。
