...
> 更新：2026-09-06（执行方向修订：证据梯子 → 可用优先；P2-USE 五个切片已全部完成；P2-FE-2 场景 1-3 合同已冻结；P2-FE-3 已完成材料/QA/练习正式证据迁移；P2-FE-4 本轮完成报告预览正式入口迁移；活动主线继续为 **P2-FE（前后端场景对齐）**）
...
- [ ] P2-FE-3 / P2-FE-4：按合同实施并迁移剩余 legacy 操作。**进行中：场景 2、场景 3 已完成当前 scoped 证据；P2-FE-4 已迁移材料重命名，以及场景 1 的目标/模块查看、重命名、归档、依赖删除与只读学习节奏 JSON 导出。**
  ...
  P2-FE-4 当前完成项：材料重命名保持既有 `implement_then_migrate` 结果；目标/模块查看、重命名、归档、计划依赖删除和只读 rhythm export 已迁移到 `/app/plans.html`；本轮将既有 `GET /api/study/reports/{report_id}/preview` 接入 `/app/reports.html` 的“刷新报告预览”按钮。预览只渲染脱敏摘要，失败显示安全文案并可重试，busy 时禁止重复请求，刷新后从服务端恢复。新增 `browser_p2_fe4_report_preview_app.spec.js`（2 passed）；plans focused browser 仍为 `5 passed`，Phase 9A API/domain focused backend 为 `12 passed`。无 API/schema/migration 变化；purge、delivery 继续保持 `intentional/not_exposed`。
  尚未完成：
  1. **剩余操作级差异分类与迁移**：当前有 23 个 spec 文件包含 `/legacy`，但它们混合兼容入口、已由 `/app` 覆盖的旧回归、opt-in real smoke 和真实缺口；不能沿用初始“19 spec / 56 test”作为当前剩余量。目标/模块管理、依赖删除、rhythm export 和报告预览已迁移；下一批继续从剩余候选中按真实用户价值和 API/UI 差异选择。
