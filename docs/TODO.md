...
- [ ] ARK、硅基流动、Agnes AI-Hub、Sub2API 逐个完成脱敏 capability matrix 和真实验收（API/UI smoke 现强制 target 与 runtime provider 一致；Agnes 有多个本地命名 model profile，但每个进程只选择一个 profile，且缺少已验证 runtime target/model/key，未发送真实请求，provider-specific real evidence 仍 pending）。
