...
- [ ] B2：真实 OCR 已选 PaddleOCR 主路径、RapidOCR ONNX 回退（Tesseract 仅兼容后备）；RapidOCR ONNX 已完成 Composer C1 smoke（精确为 `rapidocr_onnxruntime==1.4.4` 及其 bundled ONNX 模型、Windows/Python 3.10，`12/12` 通过），证据：`H:\studybuddy-composer\results\ocr-rapidocr\c1-smoke.json`。PaddleOCR 仅完成 package preflight，尚未完成本地模型 C1；B2 的 C2 Integration 与 C3-C6 Formal `ImageOcrProvider` 仍待执行。结果必须 draft-first 并需用户确认。
