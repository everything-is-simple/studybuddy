...
- [ ] B4：QQ SMTP、飞书 Webhook 分别完成 Composer smoke -> Integration -> Formal dry-run/live 门禁；live 默认关闭、逐次确认、allowlist 和 explicit opt-in。B4 C3 Formal contract freeze 已完成：`docs/contracts/B4_DELIVERY_COMPONENT_CONTRACT.md`；脱敏证据：`docs/evidence/B4_DELIVERY_C3_CONTRACT_EVIDENCE.md`。B4 C4 已完成 `implemented/backend-pass`：Formal 独立 SMTP/Feishu adapter、运行时分渠道 target mapping、allowlist、大小限制、脱敏 provider error mapping、idempotency/retry 与默认 off/no-network/live gate 测试；证据：`docs/evidence/B4_DELIVERY_C4_IMPLEMENTATION_EVIDENCE.md`。Formal live 仍关闭；C5 acceptance、C6 closeout 待完成。
