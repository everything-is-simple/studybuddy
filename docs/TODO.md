...
>
> 当前子任务状态：9B-0 已形成 `planned/audit-draft`，9B-1 已形成 `planned/contract-frozen`，9B-2 至 9B-5 已形成 `implemented/backend-pass`；审计、正式契约和 schema 边界见 [`phase9b/PHASE9B_DOMAIN_CONTRACT.md`](phase9b/PHASE9B_DOMAIN_CONTRACT.md)。正式 schema 已为 v10；9B-5 仅完成 S1 同步 repository/domain 节奏工作流与确定性 summary，API、UI、导出和 restore artifact 仍未验收。
