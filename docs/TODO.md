...
- [ ] B0（进行中）：已建立 `H:\studybuddy-composer\B0-COMPONENT-GOVERNANCE.md`、四类能力候选 catalog、统一 component-card 字段和安全/忽略规则。当前 C0 已选定 `H:\WhisperCli`/whisper.cpp `large-v3-turbo`（唯一 ASR runtime）、PaddleOCR 3.7.0 主 OCR、RapidOCR ONNX 1.4.4 回退、edge-tts 7.2.8 免费在线 TTS 候选，以及 formal-pptx + MarkItDown + 图片页 OCR 的 PPTX 三层路径；ASR 官方候选 `H:/Whisper/cli/main.exe` 已使用 `H:/Whisper/Models/ggml-large-v3-turbo.bin` 执行完整独立 C1 smoke（`C1-ASR-01` 至 `C1-ASR-14`）并通过；本机 `Whisper.dll` PE product version `1.12.0.0` 对应 GitHub `Const-me/Whisper` release `1.12.0` / commit `c5515ace19066e938854b4b99e0c2e9bbc2eeb65`。官方 release asset 哈希复核仍未验证；ASR 已通过独立 C2 Integration 与 Formal scoped contract，OCR、报告、外发候选的 C1 smoke/C2 Integration 与其 Formal 仍未通过，不能进入正式系统。ASR C1 evidence：`H:\studybuddy-composer\results\asr-whisper-cpp\c1-smoke.json`；验证命令：`C:\miniconda\py310\python.exe H:\studybuddy-composer\scripts\validate-b0-catalog.py`。
- [x] B1：真实 ASR 已在 `H:\Whisper\cli\main.exe` + `H:\Whisper\Models\ggml-large-v3-turbo.bin` 完成 Composer smoke -> Integration -> Formal `CaptureTranscriptionProvider` 独立实现；Formal 覆盖显式配置、TXT/SRT、timeout/output limit、临时文件清理、draft-first、幂等和 backup/restore non-repair。真实 runtime smoke 仅覆盖公开 `jfk.wav` 与当前 Windows 主机，官方 release asset hash 仍为 `not_verified`；不扩大为通用 ASR real-pass。
