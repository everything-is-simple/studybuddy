...
- [x] A3-FC：前端契约与架构收口。A3-FC-1 自动 endpoint/route/字段/token/retry 审计和 fixture、A3-FC-2 共享请求层与移动导航、A3-FC-3-2 全部 21 个正式静态页面的能力冻结、failure/retry 证据索引、状态/来源生命周期、360–1920、键盘、隐私 DOM、契约/治理测试均已完成。证据见 `frontend-static-capability-matrix.md` 与 `frontend-static-failure-retry-matrix.md`；已通过 backend `422 passed, 2 skipped`、当前完整 browser `126 passed, 3 skipped` 和 A3-FC 专项 `17 passed`。此关闭不等于后续 deferred capability 完成；`legacy_only`/`not_exposed`/`a3_pages` 能力仍冻结，不将 `/legacy` 证据写成静态页迁移。
- [x] A3-PAGES：已按 frontend-plan 拆分并验收 `plan-detail.html`、`note-detail.html`、`practice-session.html`、`practice-result.html`、`review.html`、`reports.html`、`settings.html`。已覆盖缺少标识、loading/empty/ready/failed/retry、来源生命周期、导航回退、只读边界、360–1920、键盘和隐私 DOM；历史 A3-PAGES gate 快照为 `117 passed, 3 skipped`，专项页面/矩阵 browser `13 passed`；当前完整 browser 基线为 `126 passed, 3 skipped`。现有 `plans.html`、`notes.html`、`practice.html`、`classroom.html`、`settings-provider.html`、`tasks.html` 继续作为兼容入口；本项不迁移后端仍未批准的写操作、报告导出/审计、Provider 配置写入或密钥保存。
- [x] A3-VISUAL：Neutral Modern token/组件/响应式视觉迁移已完成。`tokens.css`/`app.css` 已收敛 card/button/badge/notice/dialog/focus/grid；全部 21 个 `/app/*.html` 已移除局部 `<style>`。`browser_frontend_visual_matrix.spec.js` 验证 shared tokens、card、360/1920、触控目标和 focus ring（`2 passed`）；历史 A3-VISUAL closeout snapshot 为 `118 passed, 3 skipped`；当前完整 browser 基线为 `126 passed, 3 skipped`。未改 API、写操作范围或业务语义，且未迁移任何 deferred capability。
- [x] Practice workflow 第二阶段 scoped UI slice：`practice.html` 已提供会话/错题导航；`practice-session.html` 完成公开题目、start/submit/finish、嵌套结果、expired/source warning、retry/stale 安全边界；`practice-result.html` 使用 `summary.score_total`/`summary.total_item_count`；`review.html` 完成详情、feedback、review、mark-mistake、redo、archive 操作及安全刷新。`browser_practice_workflow.spec.js` `7 passed`，Phase 9C focused backend `19 passed`。完整 backend `422 passed, 2 skipped`；完整 browser `127 passed, 3 skipped`，已通过 file-import/material-search/9B/9C 服务生命周期串行修复收口。3 个 skip 均为 opt-in real-provider browser smoke；该结果为限定范围 scoped closeout，不宣称全局 production real-pass。契约审计 `0 findings`，source-size passed。
...
- [x] Practice 第三阶段推荐 API/数据契约设计：已冻结 recommendation-first 的只读 `GET /api/study/practice-recommendations`、limit/weak_point 查询边界、公开响应、empty/error 语义、ready/current/valid source 硬过滤、deterministic 分组排序、reason codes、session 创建复用、隐私、幂等和无 migration 结论。正式契约见 `docs/prompts/PHASE_PRACTICE_RECOMMENDATION_API_CONTRACT.md`；尚未实现 endpoint 或算法。
- [ ] B0（进行中）：已建立 `H:\studybuddy-composer\B0-COMPONENT-GOVERNANCE.md`、四类能力候选 catalog、统一 component-card 字段和安全/忽略规则。当前 C0 已选定 `H:\WhisperCli`/whisper.cpp `large-v3-turbo`（唯一 ASR runtime）、PaddleOCR 3.7.0 主 OCR、RapidOCR ONNX 1.4.4 回退、edge-tts 7.2.8 免费在线 TTS 候选，以及 formal-pptx + MarkItDown + 图片页 OCR 的 PPTX 三层路径；仅 ASR CLI、本地 Python 运行时、PPTX 文本转换等预检通过，候选独立 C1 smoke、Integration 与 Formal 仍未通过，不能进入正式系统。验证命令：`C:\miniconda\py310\python.exe H:\studybuddy-composer\scripts\validate-b0-catalog.py`。
