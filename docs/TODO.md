...
- [ ] B3：报告能力切片。C0 候选审计、C1 Composer smoke、C2 isolated Integration、C3 Formal contract freeze、C4 Formal focused implementation 与 C5 Formal acceptance 已完成，范围为复用现有 Phase 9D domain 的 local deterministic JSON/Markdown projection；`report-core` 在声明 synthetic scope 内为 `integration_passed`，Formal C4 已补齐 1 MiB JSON/Markdown export limit 和稳定 `payload_too_large` 错误，C5 已验证 API、reports.html、source lifecycle、backup/restore 与 operator no-side-effect 边界。C3 contract 见 `docs/contracts/B3_REPORT_COMPONENT_CONTRACT.md`，C5 evidence 见 `docs/evidence/B3_REPORT_C5_ACCEPTANCE_EVIDENCE.md`。PDF、HTML/email、Feishu card、AI narrative、delivery state、网络与 task scheduling 均排除。下一步是 B3 C6 scoped closeout；继续保持 `delivery=off`、allowlisted dry-run、append-only audit，绝不把 dry-run 标为已发送，也不自动批准 live delivery。
