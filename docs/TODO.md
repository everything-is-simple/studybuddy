...
- [x] A3-VISUAL：Neutral Modern token/组件/响应式视觉迁移已完成。`tokens.css`/`app.css` 已收敛 card/button/badge/notice/dialog/focus/grid；全部 21 个 `/app/*.html` 已移除局部 `<style>`。`browser_frontend_visual_matrix.spec.js` 验证 shared tokens、card、360/1920、触控目标和 focus ring（`2 passed`）；完整 browser `118 passed, 3 skipped`。未改 API、写操作范围或业务语义，且未迁移任何 deferred capability。
