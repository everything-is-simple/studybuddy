...
- [ ] B0（进行中）：已建立 `H:\studybuddy-composer\B0-COMPONENT-GOVERNANCE.md`、四类能力候选 catalog、统一 component-card 字段和安全/忽略规则；候选独立 smoke、Integration 与 Formal 仍未通过，不能进入正式系统。验证命令：`C:\miniconda\py310\python.exe H:\studybuddy-composer\scripts\validate-b0-catalog.py`。
