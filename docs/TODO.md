...
- [ ] ARK、硅基流动、Agnes AI-Hub、Sub2API 逐个完成脱敏 capability matrix 和真实验收（API/UI smoke 现强制 target 与 runtime provider 一致；Agnes 已有独立 child-process launcher/runbook，但缺少 matching runtime provider/target/model/base URL，未发送真实请求，provider-specific real evidence 仍 pending）。
