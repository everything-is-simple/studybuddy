...
> 更新：2026-08-31（B4 Delivery C5 scoped acceptance evidence 已同步）
...
证据：`H:\studybuddy-test\artifacts\infrastructure-i4\latest.json` 和 `latest.md`（最近一次基线已重新运行并更新）。当前 backend 全测：`413 passed, 2 skipped`；2 个 skip 为默认关闭的真实 Provider smoke。
...
> 当前状态：9D-0 为 `planned/audit-draft`，结论是只立项 deterministic fake/loopback OCR/ASR、本地脱敏报告和 delivery dry-run；9D-1 为 `planned/contract-frozen`；9D-2 至 9D-8 为 `implemented/backend-pass`，9D-9 为 `browser-pass`，9D-10 为 `backend-pass`，9D-11 为 `closeout-scoped-pass`。
...
- [x] A3-FC：前端契约与架构收口。A3-FC-1 自动 endpoint/route/字段/token/retry 审计和 fixture、A3-FC-2 共享请求层与移动导航、A3-FC-3-2 全部 21 个正式静态页面的能力冻结、failure/retry 证据索引、状态/来源生命周期、360–1920、键盘、隐私 DOM、契约/治理测试均已完成。证据见 `frontend-static-capability-matrix.md` 与 `frontend-static-failure-retry-matrix.md`；已通过 backend `426 passed, 2 skipped`、当前完整 browser `130 passed, 3 skipped` 和 A3-FC 专项 `17 passed`。此关闭不等于后续 deferred capability 完成；`legacy_only`/`not_exposed`/`a3_pages` 能力仍冻结，不将 `/legacy` 证据写成静态页迁移。
- [x] A3-PAGES：已按 frontend-plan 拆分并验收 `plan-detail.html`、`note-detail.html`、`practice-session.html`、`practice-result.html`、`review.html`、`reports.html`、`settings.html`。已覆盖缺少标识、loading/empty/ready/failed/retry、来源生命周期、导航回退、只读边界、360–1920、键盘和隐私 DOM；历史 A3-PAGES gate 快照为 `117 passed, 3 skipped`，专项页面/矩阵 browser `13 passed`；当前完整 browser 基线为 `130 passed, 3 skipped`。现有 `plans.html`、`notes.html`、`practice.html`、`classroom.html`、`settings-provider.html`、`tasks.html` 继续作为兼容入口；本项不迁移后端仍未批准的写操作、报告导出/审计、Provider 配置写入或密钥保存。
- [x] A3-VISUAL：Neutral Modern token/组件/响应式视觉迁移已完成。`tokens.css`/`app.css` 已收敛 card/button/badge/notice/dialog/focus/grid；全部 21 个 `/app/*.html` 已移除局部 `<style>`。`browser_frontend_visual_matrix.spec.js` 验证 shared tokens、card、360/1920、触控目标和 focus ring（`2 passed`）；历史 A3-VISUAL closeout snapshot 为 `118 passed, 3 skipped`；当前完整 browser 基线为 `130 passed, 3 skipped`。未改 API、写操作范围或业务语义，且未迁移任何 deferred capability。
- [x] Practice workflow 第二阶段 scoped UI slice：`practice.html` 已提供会话/错题导航；`practice-session.html` 完成公开题目、start/submit/finish、嵌套结果、expired/source warning、retry/stale 安全边界；`practice-result.html` 使用 `summary.score_total`/`summary.total_item_count`；`review.html` 完成详情、feedback、review、mark-mistake、redo、archive 操作及安全刷新。`browser_practice_workflow.spec.js` `7 passed`，Phase 9C focused backend `19 passed`。完整 backend `426 passed, 2 skipped`；完整 browser `130 passed, 3 skipped`，已通过 file-import/material-search/9B/9C 服务生命周期串行修复收口。3 个 skip 均为 opt-in real-provider browser smoke；该结果为限定范围 scoped closeout，不宣称全局 production real-pass。契约审计 `0 findings`，source-size passed。
...
- [ ] TTS（独立重新立项）：edge-tts 7.2.8 是免费但在线的显式用户操作候选，不购买 API Key、不属于当前 Phase 9D 批准范围；先冻结网络 opt-in、音频保留/清理、隐私和失败契约，再决定 Composer -> Integration -> Formal。
...
- [ ] B4：QQ SMTP、飞书 Webhook 分别完成 Composer smoke -> Integration -> Formal dry-run/live 门禁；live 默认关闭、逐次确认、allowlist 和 explicit opt-in。B4 C3 Formal contract freeze 已完成：`docs/contracts/B4_DELIVERY_COMPONENT_CONTRACT.md`；脱敏证据：`docs/evidence/B4_DELIVERY_C3_CONTRACT_EVIDENCE.md`。B4 C4 已完成 `implemented/backend-pass`：Formal 独立 SMTP/Feishu adapter、运行时分渠道 target mapping、allowlist、大小限制、脱敏 provider error mapping、idempotency/retry 与默认 off/no-network/live gate 测试；证据：`docs/evidence/B4_DELIVERY_C4_IMPLEMENTATION_EVIDENCE.md`。B4 C5 已完成 `scoped acceptance passed`：default-off/live-blocked browser、source lifecycle、backup/restore no-send 与独立 operator-authorized SMTP/Feishu synthetic smoke 均通过；证据：`docs/evidence/B4_DELIVERY_C5_ACCEPTANCE_EVIDENCE.md`。Formal product API live 仍关闭；C6 closeout 待完成。
