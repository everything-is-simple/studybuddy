...
- [x] Practice workflow 第三阶段需求审计/契约冻结：已审计现有 exercise/attempt/review/mistake/weak-point/source lifecycle 与 task runner 边界，冻结自适应出题、间隔重复、人工简答复核的状态机、时区、隐私、幂等、失败/retry 和 backup/restore 语义。正式契约见 `docs/prompts/PHASE_PRACTICE_WORKFLOW_PHASE3_AUDIT_AND_CONTRACT.md`；本阶段不新增 schema、migration、endpoint、错误码、算法或后台调度。
- [x] Practice 第三阶段推荐 API/数据契约设计：已冻结 recommendation-first 的只读 `GET /api/study/practice-recommendations`、limit/weak_point 查询边界、公开响应、empty/error 语义、ready/current/valid source 硬过滤、deterministic 分组排序、reason codes、session 创建复用、隐私、幂等和无 migration 结论。正式契约见 `docs/prompts/PHASE_PRACTICE_RECOMMENDATION_API_CONTRACT.md`；API、算法和前端选择工作区均已有 focused backend/browser evidence。
- [ ] B0（进行中）：已建立 `H:\studybuddy-composer\B0-COMPONENT-GOVERNANCE.md`、四类能力候选 catalog、统一 component-card 字段和安全/忽略规则。当前 C0 已选定 `H:\WhisperCli`/whisper.cpp `large-v3-turbo`（唯一 ASR runtime）、PaddleOCR 3.7.0 主 OCR、RapidOCR ONNX 1.4.4 回退、edge-tts 7.2.8 免费在线 TTS 候选，以及 formal-pptx + MarkItDown + 图片页 OCR 的 PPTX 三层路径；ASR 已完成 C1、C2 与 Formal API/static-browser smoke，但 C5/C6 被完整 Chromium baseline 的两个非 ASR 失败阻塞。OCR、报告、外发候选的 C1 smoke/C2 Integration 与其 Formal 仍未通过，不能进入正式系统。ASR C1 evidence：`H:\studybuddy-composer\results\asr-whisper-cpp\c1-smoke.json`；Formal evidence：`docs/prompts/FORMAL_ASR_ACCEPTANCE_EVIDENCE.md`；验证命令：`C:\miniconda\py310\python.exe H:\studybuddy-composer\scripts\validate-b0-catalog.py`。
- [ ] B1（C5/C6 收口中）：真实 ASR 已在 `H:\Whisper\cli\main.exe` + `H:\Whisper\Models\ggml-large-v3-turbo.bin` 完成 Composer smoke -> Integration -> Formal provider/API/backup-restore 与 opt-in static-browser smoke。Formal 覆盖显式配置、TXT/SRT、timeout/output limit、临时文件清理、draft-first、幂等、真实 API lifecycle、backup/restore non-repair，以及静态 `capture.html` 的上传 → Whisper 转写 → 草稿 → 用户确认 Chromium 路径；capability 与 UI 不暴露 runtime/model 路径，ASR 未接入 task runner。完整 Chromium suite 有两个独立可复现的非 ASR 失败（`browser_file_import.spec.js`、`browser_phase9c.spec.js`），因此 C5 full-browser gate 和 C6 文档 closeout 尚未通过。真实 runtime/browser smoke 仅覆盖公开 `jfk.wav` 与当前 Windows 主机，官方 release asset hash、其它格式/语言/环境、取消/子进程树清理、并发/容量仍为 `not_verified`；不扩大为通用 ASR real-pass。证据：`docs/prompts/FORMAL_ASR_ACCEPTANCE_EVIDENCE.md`。
