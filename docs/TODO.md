...
- [ ] 9A-6：完成 delete/restore/purge/re-index/source unavailable/stale 的 source lifecycle 集成；scoped backend/browser gates 已通过（focused `16 passed`、full backend `270 passed, 2 skipped`、Phase 9A Chromium `3 passed`），证据草案见 [`PHASE9A_SOURCE_LIFECYCLE_EVIDENCE.md`](PHASE9A_SOURCE_LIFECYCLE_EVIDENCE.md)，待 9A-8 closeout 后再最终收口。
- [x] 9A-7：完成 9A 数据 backup/verify/restore、历史保留和 non-repair 验收；`backend/tests/test_phase9a_backup_restore.py` 与既有 backup/restore/restore-acceptance tests 通过，focused `13 passed`，full backend `272 passed, 2 skipped`。状态为 `restore-gates-pass`，9A-8 acceptance/documentation closeout 仍未完成。证据见 [`PHASE9A_BACKUP_RESTORE_EVIDENCE.md`](PHASE9A_BACKUP_RESTORE_EVIDENCE.md)。
...
>
> 9A-0 已完成 `planned/audit-draft`，9A-1 已完成 `planned/contract-frozen`，9A-2/9A-3/9A-4 已完成 `implemented/backend-pass`，9A-5 已完成 `browser-pass`，9A-6 scoped backend/browser gates 已通过但尚未 closeout，9A-7 已完成 `restore-gates-pass`。当前 full backend 为 `272 passed, 2 skipped`，Phase 9A Chromium 为 `3 passed`。9A-8 全量 evidence 与 acceptance closeout 仍未完成。
