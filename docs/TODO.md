...
- [ ] B2：真实 OCR 已选 PaddleOCR 主路径、RapidOCR ONNX 回退（Tesseract 仅兼容后备）；PaddleOCR 与 RapidOCR ONNX 均完成 Composer C1 smoke（精确为 PaddleOCR 3.7.0/PaddlePaddle 3.3.1 + PP-OCRv5_server_det/rec、RapidOCR 1.4.4 bundled ONNX、Windows/Python 3.10，均 `12/12` 通过）。PaddleOCR C2 Integration 已通过（原件 hash/storage、draft-first confidence/uncertain、确认后同 material revision、source lifecycle、rollback、backup/restore non-repair）。B2 OCR C3 Formal contract freeze 已完成，正式契约见 `docs/contracts/B2_IMAGE_OCR_PROVIDER_CONTRACT.md`，脱敏索引见 `docs/evidence/B2_OCR_C3_CONTRACT_EVIDENCE.md`；B2 C4 Formal backend implementation 已完成限定范围：独立 PaddleOCR adapter、显式 OCR capability gate、服务端图片输入/输出约束、image-only provider selection，以及现有 capture draft boundary 的 focused backend coverage；B2 C5 已完成 focused backend/browser gates 和显式 opt-in 本地 synthetic PNG smoke，证据见 `docs/evidence/B2_OCR_C5_ACCEPTANCE_EVIDENCE.md`；C6 仍待执行。结果必须 draft-first 并需用户确认。
