...
- [x] notes → note-detail A 类纯用户路径审查与功能交付（2026-09-11，待 GPT 二审）：修复 8 个真实缺陷——①【后端】全新 data root 第一笔用户笔记创建必然失败（create_note 缺少惰性建默认项目，计划/模块域均有）②note-detail.html 死页面（无任何 UI 入口）③note-detail 读取不存在的字段导致正文永不渲染④notes.html 来源状态恒「未关联来源」⑤AI 草稿生成要求手输 UI 不可见的材料 ID（改为材料下拉）⑥生成成功后草稿永不选中展示（返回形状不匹配）⑦busy 解锁覆盖状态流转后的编辑禁用（data-busy-keep）⑧列表项键盘处理器吞掉详情按钮的 Enter（键盘不可达）。新增 A 类 E2E `browser_notes_userpath.spec.js` 8 passed×2（UI 建数据、跨页跳转、失败注入恢复、真重启持久化、窄屏、键盘）；相关回归 18 passed；后端全量 629 passed, 3 skipped；契约审计 0 findings；源码体积与 diff-check 通过。两页状态 `tested`（倾向 e2e-real-pass，待二审）。未验证：真实 Provider、真实 OCR/ASR、vector/hybrid 检索、JSON 导出、完整键盘审计、屏幕阅读器。
