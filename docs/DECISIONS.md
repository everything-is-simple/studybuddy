...
- Synchronous `POST /api/qa/ask` persists a running `ai_operations` record and user message before retrieval/provider work. On success it atomically writes the assistant message, ready cited answer, verified citations and succeeded operation. Provider calls happen outside the final SQLite write transaction. Only citation keys emitted by assembled context and revalidated server-side may persist; a provider-forged or unavailable key fails generation without an answer artifact. Q&A UI and purge-driven historical `source_unavailable` citation state remain separate work.
