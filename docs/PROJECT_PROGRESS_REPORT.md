...
- ARK、硅基流动、Agnes AI-Hub、Sub2API 已建立独立 provider capability matrix 和脱敏 opt-in 验收命令；API/UI smoke 现在要求 explicit target 与 runtime provider 一致。ARK 缺少匹配的 model ID 和 HTTPS base URL，因此未发送真实请求并保持 `not_verified`；其它 Provider 也仍待各自验证。同步 Q&A 已实现显式 `Idempotency-Key` 成功 replay、running 冲突、失败重试和请求触发的 5 分钟 lease stale recovery；后台扫描、cancel、跨进程协调和真实断电恢复仍未实现。
