# 前端契约自动审计报告

> 此报告由 `backend/scripts/audit-frontend-contract.py` 生成；发现项是待修复或待确认事项，不是完成声明。

- 页面：21
- 后端路由：168
- 发现项：0

## 发现项

| 类型 | 页面 | 值 |
|---|---|---|
| — | — | 未发现问题 |

## 页面扫描

| 页面 | API 数 | 直接 fetch | 写操作 | retry 信号 | scope |
|---|---:|---|---|---|---|
| `capture.html` | 8 | 否 | 是 | 是 | 是 |
| `cards.html` | 3 | 否 | 是 | 是 | 是 |
| `classroom.html` | 6 | 否 | 是 | 是 | 是 |
| `exercises.html` | 4 | 否 | 是 | 是 | 是 |
| `index.html` | 0 | 否 | 否 | 是 | 否 |
| `material-detail.html` | 2 | 否 | 是 | 是 | 是 |
| `materials.html` | 5 | 否 | 是 | 是 | 是 |
| `note-detail.html` | 1 | 否 | 否 | 是 | 是 |
| `notes.html` | 0 | 否 | 是 | 是 | 是 |
| `plan-detail.html` | 1 | 否 | 否 | 是 | 是 |
| `plans.html` | 0 | 否 | 是 | 是 | 是 |
| `practice-result.html` | 0 | 否 | 否 | 是 | 是 |
| `practice-session.html` | 1 | 否 | 是 | 是 | 是 |
| `practice.html` | 6 | 否 | 是 | 是 | 是 |
| `qa.html` | 4 | 否 | 是 | 是 | 是 |
| `reports.html` | 1 | 否 | 否 | 是 | 是 |
| `review.html` | 2 | 否 | 是 | 是 | 是 |
| `settings-provider.html` | 5 | 否 | 是 | 是 | 是 |
| `settings.html` | 4 | 否 | 是 | 是 | 是 |
| `tasks.html` | 4 | 否 | 是 | 是 | 是 |
| `today.html` | 3 | 否 | 否 | 否 | 是 |
