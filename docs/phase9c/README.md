...
本目录保存 Phase 9C 的总体规划、共用上下文、逐子任务执行 prompt、推荐执行顺序和验收门禁。Phase 9C 整体仍为 `planned`：9C-2/9C-3/9C-4 已分别达到 `implemented/backend-pass`，但 prompt、契约草案或历史项目类似功能本身不是实现证据，且不得标记 Phase 9C completed。
