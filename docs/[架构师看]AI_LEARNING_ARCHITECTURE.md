...
正式实现已支持 SQLite FTS5 lexical retrieval、显式 embedding indexing、vector cosine retrieval、deterministic hybrid RRF 和明确 fallback policy。embedding identity、f32 payload codec、ready/stale lifecycle、indexing lease 与 retry 的正式契约见 [`contracts/PHASE7_1_AUDIT_AND_CONTRACT.md`](contracts/PHASE7_1_AUDIT_AND_CONTRACT.md)；外部 real-pass 仅限 [`evidence/PHASE7_EMBEDDING_ACCEPTANCE_EVIDENCE.md`](evidence/PHASE7_EMBEDDING_ACCEPTANCE_EVIDENCE.md) 所记录的 Mistral 精确配置。
