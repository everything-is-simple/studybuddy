...
状态：material revision、deterministic chunks、lexical/vector/hybrid retrieval、context/citation、Q&A、Cards/Exercises，以及 Phase 9A/9B/9C 已按各自证据实现并验收；Phase 9D 的部分立项范围已完成 v12 schema、共享 domain、S7 deterministic fake/loopback capture/transcription、S7→S2 confirmed transcript ingestion、S6 report aggregation/redaction、默认关闭的 allowlisted dry-run delivery audit、API、Chromium workspace、source lifecycle 和 backup/restore non-repair。schema 由 Phase 9A 的 v9、Phase 9B 的 v10、Phase 9C 的 v11、Phase 9D 的 v12 连续演进到当前 v13（Phase 10-2 task/attempt persistence schema；无 runner）；各阶段完成声明分别限于对应 acceptance 文档记录的 deterministic fake-provider/loopback、本地单进程 SQLite、Chromium 与 backup/restore 范围，证据见 [`evidence/PHASE9A_ACCEPTANCE_EVIDENCE.md`](../.archive/evidence/PHASE9A_ACCEPTANCE_EVIDENCE.md)、[`evidence/PHASE9B_ACCEPTANCE_EVIDENCE.md`](../.archive/evidence/PHASE9B_ACCEPTANCE_EVIDENCE.md)、[`evidence/PHASE9C_ACCEPTANCE_EVIDENCE.md`](../.archive/evidence/PHASE9C_ACCEPTANCE_EVIDENCE.md)、[`evidence/PHASE9D_ACCEPTANCE_EVIDENCE.md`](../.archive/evidence/PHASE9D_ACCEPTANCE_EVIDENCE.md)。S6 report 仅支持显式生成的 daily/weekly/monthly/exam_alert 安全聚合、snapshot/fingerprint、JSON/Markdown 导出和 source degradation；delivery 默认为 off，dry-run 不联网，live 固定拒绝。Phase 9D 不包含真实 OCR/ASR、TTS、真实交付或全局 production `real-pass`；S2 ingestion 仅支持显式 draft edit/confirm/reject 后的 backend 接入。C0 已选 `H:\WhisperCli`/whisper.cpp `large-v3-turbo`、PaddleOCR 主路径、RapidOCR ONNX 回退、edge-tts 与 PPTX 三层转换方案，但本机 CLI/运行时预检不构成真实 provider、C1/C2、Formal 或 capability 完成证据；详见 [`contracts/MEDIA_CAPABILITY_DECISION.md`](../.archive/contracts/MEDIA_CAPABILITY_DECISION.md)。
...
正式实现已支持 SQLite FTS5 lexical retrieval、显式 embedding indexing、vector cosine retrieval、deterministic hybrid RRF 和明确 fallback policy。embedding identity、f32 payload codec、ready/stale lifecycle、indexing lease 与 retry 的正式契约见 [`contracts/PHASE7_1_AUDIT_AND_CONTRACT.md`](../.archive/contracts/PHASE7_1_AUDIT_AND_CONTRACT.md)；外部 real-pass 仅限 [`evidence/PHASE7_EMBEDDING_ACCEPTANCE_EVIDENCE.md`](../.archive/evidence/PHASE7_EMBEDDING_ACCEPTANCE_EVIDENCE.md) 所记录的 Mistral 精确配置。
