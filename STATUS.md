...
状态含义：`not started`、`researching`、`implemented`、`real-pass`。当前已通过 FastAPI multipart 上传、原文件保存、Parser、SQLite 写入、服务重启后 API 回读；由于尚未完成完整浏览器验收、多文件流程和异常恢复测试，仍只能标记为 `implemented`，不能标记 `real-pass`。
