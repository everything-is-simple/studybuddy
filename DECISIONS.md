...
- 现阶段不创建 AI migration 或表，先完成 schema/Protocol/fake-provider/citation contract 设计；设计文档为 `docs/ai-learning-architecture.md`。
...
- A process-local SHA-256 keyed lock coordinates same-hash imports after staging/digest calculation. It covers create/reuse, parser, SQLite persist and newly-created-original failure cleanup, so a failed request cannot remove an original for a waiting or successful same-hash request. Different hashes do not use a global upload lock. This is explicitly single-process only and is not a cross-process, multi-instance, or network-filesystem locking protocol. Process crash recovery is tested with a controlled subprocess and fixed tokens: pre-commit transactions roll back, original-only files follow strict orphan recovery, committed rows remain readable after restart. This is not a real power-loss, disk-corruption, hardware-failure, or network-filesystem recovery claim.
