...
- The embedded browser page renders search `snippet` and `match_fields` with text nodes, never HTML interpolation, and hides them outside active search results. Backend remains capped at 160 characters and list responses never include full text. Active/deleted lifecycle behavior is unchanged.
