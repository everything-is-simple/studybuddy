...
- The embedded browser page renders search `snippet`, `match_fields` and detail content with text nodes, never HTML interpolation, and hides search context outside active search results. Active search detail safely marks the first body match; name-only matches do not fabricate body highlights. Backend remains capped at 160 characters and list responses never include full text. Active/deleted lifecycle behavior is unchanged.
