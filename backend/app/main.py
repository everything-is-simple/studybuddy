...
document.querySelector('#form').onsubmit=async event=>{event.preventDefault();await importFiles([...document.querySelector('#file').files],'file')};document.querySelector('#folder-import').onclick=async()=>{await importFiles([...document.querySelector('#folder').files],'folder')};filters();loadProviderCapabilities();loadList();refreshQaHistory();try{const restoredThread=sessionStorage.getItem('studybuddy.qa.thread');if(restoredThread)loadQaHistory(restoredThread)}catch(_){} </script></body></html>"""
