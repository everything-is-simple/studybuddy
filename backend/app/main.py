...
async function refreshPhase9c(){const generation=++phase9cGeneration;try{const [sessions,goals,exercises]=await Promise.all([phase9cJson('/api/study/practice-sessions'),phase9cJson('/api/study/cram-goals'),phase9cJson('/api/study/exercises')]);if(generation!==phase9cGeneration)return;phase9cSessions=sessions||[];phase9cGoals=goals||[];phase9cExercises=exercises||[];renderPhase9cSessions();renderPhase9cGoals();renderPhase9cSessionCreate();setPhase9cStatus('练习与冲刺已加载')}catch(_){if(generation===phase9cGeneration)setPhase9cStatus('练习与冲刺加载失败，可重试','error')}}
async function createPhase9cSession(){if(phase9cBusy)return;const ids=[...document.querySelectorAll('.phase9c-exercise-choice:checked')].map(node=>node.value);if(!ids.length){setPhase9cStatus('请选择至少一道 ready 练习','error');return}phase9cBusy=true;try{const session=await phase9cJson('/api/study/practice-sessions',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({title:document.querySelector('#phase9c-session-title').value.trim(),exercise_ids:ids,duration_seconds:Number(document.querySelector('#phase9c-session-duration').value),timezone:'UTC',local_date:new Date().toISOString().slice(0,10)})});await refreshPhase9c();await loadPhase9cSession(session.id)}catch(error){setPhase9cStatus(error.code==='practice_invalid_selection'?'请选择有效练习':'创建限时练习失败，可重试','error')}finally{phase9cBusy=false}}
...
async function selectPlan(id){const generation=++planGeneration;planId=id;rememberPlanId(id);renderPlanLists();setPlanStatus('正在加载计划');try{const plan=await planJson(`/api/study/plans/${encodeURIComponent(id)}`);if(generation!==planCurrent())return;renderPlanDetail(plan);await refreshRhythm(id);setPlanStatus('计划已加载')}catch(_){if(generation===planCurrent())setPlanStatus('计划加载失败，可重试','error')}}
...
function setNoteBusy(busy){noteBusy=busy;document.querySelectorAll('#notes button,#notes input,#notes textarea').forEach(node=>node.disabled=busy||node.dataset.sourceUnavailable==='true')}
...
document.querySelector('#form').onsubmit=async event=>{event.preventDefault();await importFiles([...document.querySelector('#file').files],'file')};document.querySelector('#folder-import').onclick=async()=>{await importFiles([...document.querySelector('#folder').files],'folder')};filters();loadProviderCapabilities();loadList();refreshQaHistory();refreshPlans();refreshNotes();handleNavigation(); </script></body></html>"""
