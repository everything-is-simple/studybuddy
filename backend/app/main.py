...
async function loadMaterial(id,preserveQa=false){const generation=++detailGeneration;uiGeneration++;const savedThread=preserveQa?qaThreadId:null;const savedScope=preserveQa?qaScopeIds.slice():[];clearQa(preserveQa);const r=await fetch('/api/materials/'+encodeURIComponent(id));if(generation!==detailGeneration)return;if(!r.ok){statusEl.textContent='材料不可用';clearMaterial();return}const x=await r.json();if(generation!==detailGeneration)return;selectedMaterialId=id;selectedMaterial=x;qaScopeMeta.set(id,x);qaThreadId=savedThread;qaScopeIds=savedScope.length?savedScope:[id];renderQaScope();document.querySelector('#rename').disabled=mutationInFlight;document.querySelector('#delete').disabled=mutationInFlight;document.querySelector('#restore').disabled=true;document.querySelector('#download-original').disabled=mutationInFlight;document.querySelector('#export-text').disabled=mutationInFlight;document.querySelector('#open-qa').disabled=mutationInFlight;document.querySelector('#title').textContent=x.original_name;const context=searchContext(x.original_name,x.text||'');document.querySelector('#meta').textContent=`${x.status} · ${x.parser_id} ${x.parser_version} · SHA-256 ${x.source_sha256}${context?' · '+context:''}`;document.querySelector('#warnings').textContent=x.error_code?`error_code: ${x.error_code} · ${x.warnings.join(' ')}`:x.warnings.join(' ');document.querySelector('#spans').textContent=`spans: ${x.spans.length} · ${x.spans.map(s=>s.label).join(', ')}`;const displayText=x.text||x.spans.filter(s=>s.text.trim()).map(s=>`[${s.label}]\\n${s.text}`).join('\\n\\n')||'没有可显示的正文';renderDetailContent(displayText);writeNavigation(true);await refreshQaIndex();if(savedThread)await loadQaHistory(savedThread)}
