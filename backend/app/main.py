...
<body><main><h1>StudyBuddy 文件导入</h1><form id="form"><input id="file" type="file" multiple required><button type="submit">导入文件</button></form><div id="status" class="status"></div><div id="summary" class="summary"></div><div id="batch-items"></div><section class="layout"><aside><h2>材料</h2><div id="views" class="filters"><button id="active-view" type="button">正常材料</button><button id="deleted-view" type="button">回收站</button></div><div id="filters" class="filters"></div><div id="materials"></div></aside><article><h2 id="title">选择材料</h2><div id="meta" class="meta"></div><div id="warnings" class="meta"></div><div id="spans" class="meta"></div><div id="management"><button id="rename" type="button">重命名</button><button id="delete" type="button">删除</button><button id="restore" type="button">恢复</button><button id="download-original" type="button">下载原文件</button><button id="export-text" type="button">导出解析正文</button></div><div id="content" class="content"></div></article></section></main>
...
let viewMode='active';
async function loadList(){const url=viewMode==='deleted'?'/api/materials/deleted':(currentFilter?`/api/materials?status=${currentFilter}`:'/api/materials');const r=await fetch(url);const items=await r.json();if(viewMode==='deleted'){listEl.innerHTML=items.map(x=>`<button class="item deleted-item" data-id="${x.id}">${x.original_name}<br><span class="meta">已删除 · ${x.status} · ${x.deleted_at}</span><br><span class="meta">${x.text_length} 字 · ${x.span_count} spans</span></button>`).join('')||'<p class="meta">回收站为空</p>'}else{listEl.innerHTML=items.map(x=>`<button class="item" data-id="${x.id}">${x.original_name}<br><span class="meta">${x.status} · ${x.media_type} · ${x.text_length} 字 · ${x.span_count} spans${x.error_code?' · '+x.error_code:''}</span></button>`).join('')||'<p class="meta">暂无材料</p>'}document.querySelectorAll('.item').forEach(x=>x.onclick=()=>viewMode==='deleted'?loadDeletedMaterial(x.dataset.id):loadMaterial(x.dataset.id));}
...
function setView(mode){viewMode=mode;document.querySelector('#active-view').classList.toggle('active',mode==='active');document.querySelector('#deleted-view').classList.toggle('active',mode==='deleted');filterEl.style.display=mode==='active'?'flex':'none';clearMaterial();return loadList()}
document.querySelector('#rename').onclick=renameSelected;document.querySelector('#delete').onclick=deleteSelected;document.querySelector('#restore').onclick=restoreSelected;document.querySelector('#download-original').onclick=downloadOriginal;document.querySelector('#export-text').onclick=exportText;document.querySelector('#active-view').onclick=()=>setView('active');document.querySelector('#deleted-view').onclick=()=>setView('deleted');clearMaterial();
