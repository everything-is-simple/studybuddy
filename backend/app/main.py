...
from .repository import (VALID_STATUSES, connect, get_material, get_spans, list_deleted_materials,
                         list_materials, material_state, rename_material, restore_material,
...
let viewMode='active';let currentQuery='';
async function loadList(){const url=viewMode==='deleted'?'/api/materials/deleted':`/api/materials?${new URLSearchParams(Object.fromEntries([['status',currentFilter],['q',currentQuery]].filter(([,value])=>value)))}`;const r=await fetch(url);const items=await r.json();if(viewMode==='deleted'){listEl.innerHTML=items.map(x=>`<button class="item deleted-item" data-id="${x.id}">${x.original_name}<br><span class="meta">已删除 · ${x.status} · ${x.deleted_at}</span><br><span class="meta">${x.text_length} 字 · ${x.span_count} spans</span></button>`).join('')||'<p class="meta">回收站为空</p>'}else{listEl.innerHTML=items.map(x=>`<button class="item" data-id="${x.id}">${x.original_name}<br><span class="meta">${x.status} · ${x.media_type} · ${x.text_length} 字 · ${x.span_count} spans${x.error_code?' · '+x.error_code:''}</span></button>`).join('')||'<p class="meta">暂无材料</p>'}document.querySelectorAll('.item').forEach(x=>x.onclick=()=>viewMode==='deleted'?loadDeletedMaterial(x.dataset.id):loadMaterial(x.dataset.id));}
let selectedMaterialId=null;let selectedMaterial=null;
function clearMaterial(){selectedMaterialId=null;selectedMaterial=null;document.querySelector('#title').textContent='选择材料';document.querySelector('#meta').textContent='';document.querySelector('#warnings').textContent='';document.querySelector('#spans').textContent='';document.querySelector('#content').textContent='';document.querySelector('#rename').disabled=true;document.querySelector('#delete').disabled=true;document.querySelector('#restore').disabled=true;document.querySelector('#download-original').disabled=true;document.querySelector('#export-text').disabled=true;}
async function loadMaterial(id){const r=await fetch('/api/materials/'+id);if(!r.ok){statusEl.textContent='材料不可用';clearMaterial();return}const x=await r.json();selectedMaterialId=id;selectedMaterial=x;document.querySelector('#rename').disabled=false;document.querySelector('#delete').disabled=false;document.querySelector('#restore').disabled=true;document.querySelector('#download-original').disabled=false;document.querySelector('#export-text').disabled=false;document.querySelector('#title').textContent=x.original_name;document.querySelector('#meta').textContent=`${x.status} · ${x.parser_id} ${x.parser_version} · SHA-256 ${x.source_sha256}`;document.querySelector('#warnings').textContent=x.error_code?`error_code: ${x.error_code} · ${x.warnings.join(' ')}`:x.warnings.join(' ');document.querySelector('#spans').textContent=`spans: ${x.spans.length} · ${x.spans.map(s=>s.label).join(', ')}`;document.querySelector('#content').textContent=x.text||x.spans.filter(s=>s.text.trim()).map(s=>`[${s.label}]\\n${s.text}`).join('\\n\\n')||'没有可显示的正文';}
async function loadDeletedMaterial(id){const r=await fetch('/api/materials/deleted');const items=await r.json();const x=items.find(item=>item.id===id);if(!x){statusEl.textContent='材料不可用';clearMaterial();return}selectedMaterialId=id;selectedMaterial=x;document.querySelector('#rename').disabled=true;document.querySelector('#delete').disabled=true;document.querySelector('#restore').disabled=false;document.querySelector('#download-original').disabled=true;document.querySelector('#export-text').disabled=true;document.querySelector('#title').textContent=x.original_name;document.querySelector('#meta').textContent=`已删除 · ${x.status} · 删除于 ${x.deleted_at}`;document.querySelector('#warnings').textContent=x.error_code?`error_code: ${x.error_code}`:'';document.querySelector('#spans').textContent=`spans: ${x.span_count}`;document.querySelector('#content').textContent='';}
async function restoreSelected(){if(!selectedMaterialId)return;const r=await fetch('/api/materials/'+selectedMaterialId+'/restore',{method:'POST'});const x=await r.json();if(!r.ok){statusEl.textContent=typeof x.detail==='string'?x.detail:'恢复失败';return}statusEl.textContent='材料已恢复';clearMaterial();await setView('active');await loadList();await loadMaterial(x.id)}
...
async function renameSelected(){if(!selectedMaterialId||viewMode==='deleted')return;const name=window.prompt('输入新的材料名称',selectedMaterial.original_name);if(name===null)return;const r=await fetch('/api/materials/'+selectedMaterialId,{method:'PATCH',headers:{'content-type':'application/json'},body:JSON.stringify({original_name:name})});const x=await r.json();if(!r.ok){statusEl.textContent=typeof x.detail==='string'?x.detail:'重命名失败';return}statusEl.textContent='重命名成功';await loadList();await loadMaterial(selectedMaterialId)}
async function deleteSelected(){if(!selectedMaterialId||viewMode==='deleted')return;if(!window.confirm(`确认删除材料“${selectedMaterial.original_name}”？`))return;const id=selectedMaterialId;const r=await fetch('/api/materials/'+id,{method:'DELETE'});if(!r.ok){const x=await r.json();statusEl.textContent=typeof x.detail==='string'?x.detail:'删除失败';return}statusEl.textContent='材料已删除';clearMaterial();await loadList();}
async function submitSearch(){if(viewMode==='deleted')return;currentQuery=document.querySelector('#search').value.trim();document.querySelector('#search-summary').textContent=currentQuery?`匹配 ${await countVisibleItems()}`:'';await loadList()}
async function countVisibleItems(){const url=`/api/materials?${new URLSearchParams(Object.fromEntries([['status',currentFilter],['q',currentQuery]].filter(([,value])=>value)))}`;return (await (await fetch(url)).json()).length}
...
document.querySelector('#rename').onclick=renameSelected;document.querySelector('#delete').onclick=deleteSelected;document.querySelector('#restore').onclick=restoreSelected;document.querySelector('#download-original').onclick=downloadOriginal;document.querySelector('#export-text').onclick=exportText;document.querySelector('#search-form').onsubmit=async event=>{event.preventDefault();await submitSearch()};document.querySelector('#search-clear').onclick=async()=>{document.querySelector('#search').value='';currentQuery='';document.querySelector('#search-summary').textContent='';await loadList()};document.querySelector('#active-view').onclick=()=>setView('active');document.querySelector('#deleted-view').onclick=()=>setView('deleted');clearMaterial();
function filters(){filterEl.innerHTML=['全部','成功','空文件','拒绝','失败'].map((label,i)=>`<button class="${(i?['success','empty','rejected','failed'][i-1]:'')===currentFilter?'active':''}" data-status="${i?['success','empty','rejected','failed'][i-1]:''}">${label}</button>`).join('');filterEl.querySelectorAll('button').forEach(b=>b.onclick=async()=>{currentFilter=b.dataset.status;filters();await submitSearch()})}
