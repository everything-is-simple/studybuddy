...
from .repository import (VALID_STATUSES, MAX_CONTEXT_TOKENS, connect, assemble_context, create_or_get_revision,
                         ...
                         get_qa_citation_detail, get_spans, index_material_revision, list_deleted_materials, list_materials,
...
<body><main><h1>StudyBuddy 文件导入</h1><form id="form"><input id="file" type="file" multiple required><button id="file-import" type="submit">导入文件</button></form><div><input id="folder" type="file" webkitdirectory multiple><button id="folder-import" type="button">导入文件夹</button></div><div id="status" class="status"></div><div id="summary" class="summary"></div><div id="batch-items"></div><section class="layout"><aside><h2>材料</h2><div id="views" class="filters"><button id="active-view" type="button">正常材料</button><button id="deleted-view" type="button">回收站</button></div><form id="search-form"><input id="search" type="search" placeholder="搜索材料"><button id="search-submit" type="submit">搜索</button><button id="search-clear" type="button">清除</button></form><div id="search-summary" class="meta"></div><div id="filters" class="filters"></div><div id="pagination" class="filters"><button id="page-prev" type="button">上一页</button><span id="page-info" class="meta"></span><button id="page-next" type="button">下一页</button></div><div id="batch-export" class="filters"><button id="select-all" type="button">全选当前列表</button><button id="export-selected-originals" type="button">导出选中原文件</button><button id="export-selected-text" type="button">导出选中文本</button><button id="export-selected-bundle" type="button">导出选中全部</button></div><div id="materials"></div></aside><article><h2 id="title">选择材料</h2><div id="meta" class="meta"></div><div id="warnings" class="meta"></div><div id="spans" class="meta"></div><div id="management"><button id="rename" type="button">重命名</button><button id="delete" type="button">删除</button><button id="restore" type="button">恢复</button><button id="purge" type="button">永久删除</button><button id="download-original" type="button">下载原文件</button><button id="export-text" type="button">导出解析正文</button></div><div id="content" class="content"></div><section id="qa" class="qa"><h3>材料问答</h3><div id="qa-status" class="meta">选择已建立 AI 索引的正常材料后可提问</div><textarea id="qa-question" maxlength="1000" placeholder="输入与当前材料内容匹配的问题" disabled></textarea><div><button id="ai-index" type="button" disabled>建立 AI 索引</button><button id="qa-ask" type="button" disabled>提问</button></div><div id="qa-answer" class="qa-answer"></div><div id="qa-citations" class="qa-citations"></div></section></article></section></main>
...
let selectedMaterialId=null;let selectedMaterial=null;let detailGeneration=0;let mutationInFlight=false;let uiGeneration=0;let qaInFlight=false;let qaThreadId=null;
...
function clearQa(){qaThreadId=null;qaInFlight=false;document.querySelector('#qa-question').value='';document.querySelector('#qa-question').disabled=true;document.querySelector('#qa-ask').disabled=true;document.querySelector('#ai-index').disabled=true;document.querySelector('#qa-status').textContent='选择已建立 AI 索引的正常材料后可提问';document.querySelector('#qa-answer').replaceChildren();document.querySelector('#qa-citations').replaceChildren()}
...
function setQaBusy(busy){qaInFlight=busy;document.querySelector('#qa-question').disabled=busy||viewMode!=='active'||!selectedMaterialId;document.querySelector('#qa-ask').disabled=busy||viewMode!=='active'||!selectedMaterialId;document.querySelector('#ai-index').disabled=busy||viewMode!=='active'||!selectedMaterialId}
async function refreshQaIndex(){if(!selectedMaterialId||viewMode!=='active')return;const context=currentUi();try{const r=await fetch(`/api/materials/${context.id}/ai-index`);if(!r.ok)throw Error('index_status');const data=await readJsonObject(r);if(!stillCurrent(context))return;const ready=data.status==='ready';document.querySelector('#qa-question').disabled=!ready;document.querySelector('#qa-ask').disabled=!ready;document.querySelector('#ai-index').disabled=false;document.querySelector('#qa-status').textContent=ready?'可以基于当前材料提问':data.status==='empty'?'当前材料没有可用于问答的正文':'请先建立 AI 索引'}catch(_){if(stillCurrent(context))document.querySelector('#qa-status').textContent='AI 索引状态获取失败'}}
...
async function locateCitation(key){const context=currentUi();try{const r=await fetch('/api/qa/citations/'+encodeURIComponent(key));if(!r.ok){if(stillCurrent(context))document.querySelector('#qa-status').textContent='引用不可用';return}const data=await readJsonObject(r);if(!stillCurrent(context))return;if(data.status!=='valid'||data.material_id!==selectedMaterialId||!selectedMaterial){document.querySelector('#qa-status').textContent=data.status==='source_unavailable'?'引用来源已永久删除':data.status==='source_deleted'?'引用来源已删除':'引用不可用';return}const text=selectedMaterial.text||'';if(typeof data.start_offset!=='number'||typeof data.end_offset!=='number'){document.querySelector('#qa-status').textContent='引用无法定位';return}renderCitationLocation(text,data.start_offset,data.end_offset);document.querySelector('#qa-status').textContent='已定位引用来源'}catch(_){if(stillCurrent(context))document.querySelector('#qa-status').textContent='引用定位失败'}}
...
async function askQa(){if(qaInFlight||!selectedMaterialId||viewMode!=='active')return;const question=document.querySelector('#qa-question').value.trim();if(!question){document.querySelector('#qa-status').textContent='请输入问题';return}const context=currentUi();setQaBusy(true);document.querySelector('#qa-status').textContent='正在检索并生成回答';try{const r=await fetch('/api/qa/ask',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({question,material_ids:[context.id],thread_id:qaThreadId,top_k:5})});let payload=null;try{payload=await readJsonObject(r)}catch(_){}if(!r.ok){if(stillCurrent(context))document.querySelector('#qa-status').textContent=qaMessage(payload&&payload.detail);return}if(!payload||payload.status!=='succeeded'||typeof payload.answer_text!=='string')throw Error('invalid_qa_response');if(!stillCurrent(context))return;qaThreadId=typeof payload.thread_id==='string'?payload.thread_id:null;renderQaAnswer(payload);document.querySelector('#qa-status').textContent='回答已生成'}catch(_){if(stillCurrent(context))document.querySelector('#qa-status').textContent='问答请求失败'}finally{if(stillCurrent(context))setQaBusy(false)}}
async function indexSelectedForAi(){if(qaInFlight||!selectedMaterialId||viewMode!=='active')return;const context=currentUi();setQaBusy(true);document.querySelector('#qa-status').textContent='正在建立 AI 索引';try{const r=await fetch(`/api/materials/${context.id}/ai-index`,{method:'POST'});if(!r.ok){if(stillCurrent(context))document.querySelector('#qa-status').textContent='建立 AI 索引失败';return}if(stillCurrent(context)){await refreshQaIndex();document.querySelector('#qa-status').textContent='AI 索引已建立'}}catch(_){if(stillCurrent(context))document.querySelector('#qa-status').textContent='建立 AI 索引失败'}finally{if(stillCurrent(context))setQaBusy(false)}}
async function loadMaterial(id){const generation=++detailGeneration;uiGeneration++;clearQa();const r=await fetch('/api/materials/'+id);if(generation!==detailGeneration)return;if(!r.ok){statusEl.textContent='材料不可用';clearMaterial();return}const x=await r.json();if(generation!==detailGeneration)return;selectedMaterialId=id;selectedMaterial=x;document.querySelector('#rename').disabled=mutationInFlight;document.querySelector('#delete').disabled=mutationInFlight;document.querySelector('#restore').disabled=true;document.querySelector('#download-original').disabled=mutationInFlight;document.querySelector('#export-text').disabled=mutationInFlight;document.querySelector('#title').textContent=x.original_name;const context=searchContext(x.original_name,x.text||'');document.querySelector('#meta').textContent=`${x.status} · ${x.parser_id} ${x.parser_version} · SHA-256 ${x.source_sha256}${context?' · '+context:''}`;document.querySelector('#warnings').textContent=x.error_code?`error_code: ${x.error_code} · ${x.warnings.join(' ')}`:x.warnings.join(' ');document.querySelector('#spans').textContent=`spans: ${x.spans.length} · ${x.spans.map(s=>s.label).join(', ')}`;const displayText=x.text||x.spans.filter(s=>s.text.trim()).map(s=>`[${s.label}]\\n${s.text}`).join('\\n\\n')||'没有可显示的正文';renderDetailContent(displayText);await refreshQaIndex();}
...
document.querySelector('#page-prev').onclick=async()=>{if(currentOffset===0)return;currentOffset=Math.max(0,currentOffset-PAGE_SIZE);await loadList()};document.querySelector('#page-next').onclick=async()=>{if(!currentHasMore)return;currentOffset+=PAGE_SIZE;await loadList()};document.querySelector('#select-all').onclick=()=>{const boxes=[...document.querySelectorAll('.material-select')];const checked=boxes.every(box=>box.checked);boxes.forEach(box=>box.checked=!checked)};document.querySelector('#export-selected-originals').onclick=()=>exportSelected(true,false);document.querySelector('#export-selected-text').onclick=()=>exportSelected(false,true);document.querySelector('#export-selected-bundle').onclick=()=>exportSelected(true,true);document.querySelector('#rename').onclick=renameSelected;document.querySelector('#delete').onclick=deleteSelected;document.querySelector('#restore').onclick=restoreSelected;document.querySelector('#purge').onclick=purgeSelected;document.querySelector('#download-original').onclick=downloadOriginal;document.querySelector('#export-text').onclick=exportText;document.querySelector('#qa-ask').onclick=askQa;document.querySelector('#ai-index').onclick=indexSelectedForAi;document.querySelector('#search-form').onsubmit=async event=>{event.preventDefault();await submitSearch()};document.querySelector('#search-clear').onclick=async()=>{document.querySelector('#search').value='';currentQuery='';currentOffset=0;await loadList()};document.querySelector('#active-view').onclick=()=>setView('active');document.querySelector('#deleted-view').onclick=()=>setView('deleted');clearMaterial();
...
document.querySelector('#form').onsubmit=async event=>{event.preventDefault();await importFiles([...document.querySelector('#file').files],'file')};document.querySelector('#folder-import').onclick=async()=>{await importFiles([...document.querySelector('#folder').files],'folder')};filters();loadList();</script></body></html>"""
