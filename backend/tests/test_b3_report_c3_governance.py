...
def test_b3_c3_contract_freezes_formal_reuse_and_scope():
    contract = (ROOT / ".archive/contracts/B3_REPORT_COMPONENT_CONTRACT.md").read_text(encoding="utf-8")
    evidence = (ROOT / ".archive/evidence/B3_REPORT_C3_CONTRACT_EVIDENCE.md").read_text(encoding="utf-8")
    for marker in (
...
def test_b3_c3_evidence_references_passed_candidate_gates_without_authorizing_formal():
    ...
    assert "C4 is unblocked" in (ROOT / ".archive/evidence/B3_REPORT_C3_CONTRACT_EVIDENCE.md").read_text(encoding="utf-8")
