...
test('formal app review renders weak-point summary and restores it after reload',async({page})=>{
 await page.route(`${BASE}/api/study/mistakes*`,route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({items:[],total:0,limit:20,offset:0,has_more:false})}));
...
test('formal app weak-point failure is safe and retryable without hiding mistake state',async({page})=>{
 let calls=0;
 await page.route(`${BASE}/api/study/mistakes*`,route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({items:[{id:'mistake-1',question:'公开错题',status:'open',mistake_fact:'公开原因',occurrences:[]}],total:1,limit:20,offset:0,has_more:false})}));
