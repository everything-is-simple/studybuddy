...
function startServer() { const env = {...process.env, PYTHONPATH: 'H:/studybuddy/backend', STUDYBUDDY_DATA_ROOT: RUN_ROOT}; return spawn(process.env.STUDYBUDDY_TEST_PYTHON || 'D:/miniconda/py310/python.exe', ['-m', 'uvicorn', 'app.main:app', '--host', '127.0.0.1', '--port', String(PORT)], {cwd: 'H:/studybuddy/backend', env, stdio: 'ignore', windowsHide: true}); }
...
function dbCounts() { const db = path.join(RUN_ROOT, 'studybuddy.sqlite3'); const code = `import json,sqlite3;c=sqlite3.connect(r'${db}');print(json.dumps({"materials":c.execute('SELECT COUNT(*) FROM materials').fetchone()[0],"extractions":c.execute('SELECT COUNT(*) FROM extractions').fetchone()[0],"spans":c.execute('SELECT COUNT(*) FROM text_spans').fetchone()[0]}));c.close()`; return JSON.parse(spawnSync(process.env.STUDYBUDDY_TEST_PYTHON || 'D:/miniconda/py310/python.exe', ['-c', code], {encoding: 'utf8'}).stdout); }

async function downloadFromButton(page, selector, expectedName) { const downloadPromise = page.waitForEvent('download'); await page.locator(selector).click(); const download = await downloadPromise; expect(download.suggestedFilename()).toBe(expectedName); const filePath = await download.path(); return fs.readFileSync(filePath); }

test('formal material export browser acceptance', async ({page}) => {
