...
const PORT = 8978, BASE = `http://127.0.0.1:${PORT}`, PYTHON = process.env.STUDYBUDDY_TEST_PYTHON || 'D:/miniconda/py310/python.exe';
let server;
...
test.describe.serial('tasks.html independent B-class review',()=>{
 ...
 test('B-TK-1 API pagination shape, page query and invalid filters are bounded',async({page})=>{
  ...
  await page.unroute('**/api/tasks?*');const real=await page.request.get(`${BASE}/api/tasks?limit=1&offset=0`);expect(real.status()).toBe(200);const realBody=await real.json();expect(realBody).toMatchObject({items:[],total:0,limit:1,offset:0,has_more:false});
 ...
 test('B-TK-4 cancel mutation is busy-safe, idempotent and restores after failure',async({page})=>{
  let posts=0,release,key='';const hold=new Promise(resolve=>release=resolve);await page.route('**/api/tasks/**',async route=>{if(route.request().method()==='GET')return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify(task('t-cancel','running'))});posts++;key=route.request().headers()['idempotency-key']||'';await hold;return route.fulfill({status:500,contentType:'application/json',body:'{"detail":"task_cancel_failed"}'});});await page.goto(`${BASE}/app/tasks.html?task_id=t-cancel`);const button=page.locator('[data-action="cancel"]');page.once('dialog',d=>d.accept());await button.click();await expect(button).toBeDisabled();await expect.poll(()=>posts).toBe(1);expect(key).toMatch(/^ui-/);await button.dispatchEvent('click');expect(posts).toBe(1);release();await expect(page.locator('#detail-notice')).toContainText('取消任务失败');await expect(button).toBeEnabled();await safe(page);
 });
 ...
 test('B-TK-6 list and detail failures expose independent retry controls and recover',async({page})=>{
  let listFail=true;await page.route('**/api/tasks?*',route=>listFail?route.fulfill({status:500,contentType:'application/json',body:'{"detail":"database_unavailable","path":"H:/private"}'}):route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({items:[task('recovered')],total:1,limit:25,offset:0,has_more:false})}));
  await page.goto(`${BASE}/app/tasks.html`);await expect(page.locator('#retry-list')).toBeVisible();await expect(page.locator('#error')).toContainText('数据暂时不可用');listFail=false;await page.locator('#retry-list').click();await expect(page.locator('#tasks')).toContainText('recovered');await expect(page.locator('#retry-list')).toBeHidden();await page.unroute('**/api/tasks?*');
  let detailFail=true;await page.route('**/api/tasks/recovered',route=>detailFail?route.fulfill({status:500,contentType:'application/json',body:'{"detail":"database_unavailable","traceback":"hidden"}'}):route.fulfill({status:200,contentType:'application/json',body:JSON.stringify(task('recovered','succeeded',{progress_percent:100,stage_code:'finalizing'}))}));
  await page.goto(`${BASE}/app/tasks.html?task_id=recovered`);await expect(page.locator('#retry-detail')).toBeVisible();await expect(page.locator('#detail-error')).toContainText('数据暂时不可用');detailFail=false;await page.locator('#retry-detail').click();await expect(page.locator('#task-detail')).toContainText('已成功');await expect(page.locator('#retry-detail')).toBeHidden();await safe(page);
 });
 test('B-TK-7 retry mutation disables immediately and ignores duplicate dispatch',async({page})=>{
  let posts=0,release,status='failed',key='';const hold=new Promise(resolve=>release=resolve);await page.route('**/api/tasks/**',async route=>{const method=route.request().method();if(method==='GET')return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify(task('t-retry',status,{error_code:status==='failed'?'embedding_provider_timeout':null}))});posts++;key=route.request().headers()['idempotency-key']||'';await hold;status='queued';return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify(task('t-retry','queued',{retry_count:1}))});});
  await page.goto(`${BASE}/app/tasks.html?task_id=t-retry`);const button=page.locator('[data-action="retry"]');page.once('dialog',dialog=>dialog.accept());await button.click();await expect(button).toBeDisabled();await expect.poll(()=>posts).toBe(1);expect(key).toMatch(/^ui-/);await button.dispatchEvent('click');expect(posts).toBe(1);release();await expect(page.locator('#task-detail')).toContainText('等待中');await expect(page.locator('#detail-notice')).toContainText('已加入重试队列');await safe(page);
 });
});
