...
test('formal material recycle bin browser acceptance', async ({page}) => {
  ...
  try {
    ...
    page.once('dialog', dialog => { expect(dialog.type()).toBe('confirm'); dialog.accept(); }); await page.getByRole('button', {name: '删除'}).click();
    ...
    await page.getByRole('button', {name: /same-two\.txt/}).last().click(); page.once('dialog', dialog => { expect(dialog.type()).toBe('confirm'); dialog.accept(); }); await page.getByRole('button', {name: '删除'}).click();
...
test('formal restore mutation error acceptance', async ({page}) => {
  fs.rmSync(RUN_ROOT,{recursive:true,force:true});fs.rmSync(path.dirname(ARTIFACT),{recursive:true,force:true});fs.mkdirSync(RUN_ROOT,{recursive:true});let server=startServer();const errors=[];page.on('console',m=>{if(m.type()==='error'&&!m.text().includes('Failed to load resource'))errors.push(m.text())});page.on('pageerror',e=>errors.push(e.message));try{await waitReady();await page.goto(BASE);await page.locator('input[type=file]').setInputFiles(path.join(FIXTURES,'sample.txt'));await page.getByRole('button',{name:'导入文件'}).click();await expect(page.locator('#status')).toContainText('导入完成',{timeout:30000});await page.getByRole('button',{name:/sample\.txt/}).last().click();page.once('dialog',d=>d.accept());await page.getByRole('button',{name:'删除'}).click();await page.getByRole('button',{name:'回收站'}).click();await page.getByRole('button',{name:/sample\.txt/}).last().click();await page.route(`${BASE}/api/materials/*/restore`,route=>route.fulfill({status:500,contentType:'application/json',body:'{"detail":"synthetic"}'}));await page.locator('#restore').click();await expect(page.locator('#status')).toHaveText('恢复失败');await expect(page.locator('#search-form')).toBeHidden();await expect(page.locator('#restore')).toBeEnabled();expect(errors).toEqual([])}finally{await page.unroute(`${BASE}/api/materials/*`).catch(()=>{});stopServer(server)}});
