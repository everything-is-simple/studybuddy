...
test('formal material recycle bin browser acceptance', async ({page}) => {
  ...
  try {
    ...
    await page.locator('#file').setInputFiles(inputs); await page.locator('#file-import').click();
...
test('formal material purge success, error, and duplicate protection acceptance', async ({page}) => {
  fs.rmSync(RUN_ROOT,{recursive:true,force:true});fs.mkdirSync(RUN_ROOT,{recursive:true});let server=startServer();let release;const errors=[];page.on('console',m=>{if(m.type()==='error'&&!m.text().includes('Failed to load resource'))errors.push(m.text())});page.on('pageerror',e=>errors.push(e.message));try{await waitReady();await page.goto(BASE);await page.locator('#file').setInputFiles(path.join(FIXTURES,'sample.txt'));await page.locator('#file-import').click();await expect(page.locator('#status')).toContainText('导入完成',{timeout:30000});await page.getByRole('button',{name:/sample\.txt/}).last().click();page.once('dialog',d=>d.accept());await page.getByRole('button',{name:'删除',exact:true}).click();await page.getByRole('button',{name:'回收站'}).click();await page.getByRole('button',{name:/sample\.txt/}).last().click();await expect(page.locator('#purge')).toBeEnabled();await page.route(`${BASE}/api/materials/*/purge`,route=>route.fulfill({status:500,contentType:'application/json',body:'{"detail":"synthetic"}'}));page.once('dialog',d=>d.accept());await page.locator('#purge').click();await expect(page.locator('#status')).toHaveText('永久删除失败');await expect(page.locator('#purge')).toBeEnabled();await expect(page.locator('#materials .deleted-item')).toHaveCount(1);await page.unroute(`${BASE}/api/materials/*/purge`);let count=0;await page.route(`${BASE}/api/materials/*/purge`,async route=>{count++;await new Promise(resolve=>{release=resolve});return route.continue()});page.once('dialog',d=>d.accept());await page.locator('#purge').click();await page.locator('#purge').dispatchEvent('click');await expect(page.locator('#purge')).toBeDisabled();expect(count).toBe(1);release();await expect(page.locator('#status')).toHaveText('材料已永久删除');await expect(page.locator('#materials .deleted-item')).toHaveCount(0);expect(errors).toEqual([])}finally{await page.unroute(`${BASE}/api/materials/*/purge`).catch(()=>{});if(release)release();stopServer(server)}});

test('formal restore mutation error acceptance', async ({page}) => {
  fs.rmSync(RUN_ROOT,{recursive:true,force:true});fs.rmSync(path.dirname(ARTIFACT),{recursive:true,force:true});fs.mkdirSync(RUN_ROOT,{recursive:true});let server=startServer();const errors=[];page.on('console',m=>{if(m.type()==='error'&&!m.text().includes('Failed to load resource'))errors.push(m.text())});page.on('pageerror',e=>errors.push(e.message));try{await waitReady();await page.goto(BASE);await page.locator('#file').setInputFiles(path.join(FIXTURES,'sample.txt'));await page.locator('#file-import').click();await expect(page.locator('#status')).toContainText('导入完成',{timeout:30000});await page.getByRole('button',{name:/sample\.txt/}).last().click();page.once('dialog',d=>d.accept());await page.getByRole('button',{name:'删除',exact:true}).click();await page.getByRole('button',{name:'回收站'}).click();await page.getByRole('button',{name:/sample\.txt/}).last().click();await page.route(`${BASE}/api/materials/*/restore`,route=>route.fulfill({status:500,contentType:'application/json',body:'{"detail":"synthetic"}'}));await page.locator('#restore').click();await expect(page.locator('#status')).toHaveText('恢复失败');await expect(page.locator('#search-form')).toBeHidden();await expect(page.locator('#restore')).toBeEnabled();expect(errors).toEqual([])}finally{await page.unroute(`${BASE}/api/materials/*`).catch(()=>{});stopServer(server)}});
