...
test('all static pages have safe initial states at mobile and desktop widths',async({page})=>{for(const [name,status] of PAGES){for(const width of [360,1920]){await page.setViewportSize({width,height:844});await page.goto(`${BASE}/app/${name}`);await expect(page.locator('main')).toBeVisible();await expect.poll(()=>page.evaluate(()=>document.documentElement.scrollWidth<=document.documentElement.clientWidth)).toBe(true);if(status)await expect(page.locator(status)).not.toContainText(/正在加载|加载中|正在检查|检查中/,{timeout:5000});if(width===360){const toggle=page.getByRole('button',{name:'更多'});await expect(toggle).toBeVisible();await toggle.press('Enter');await expect(page.locator('#primary-navigation')).toHaveClass(/is-open/)}await expect(page.locator('body')).not.toContainText(/H:\\|SELECT\s|Traceback|api[_-]?key|secret|stored_path/i)}}});
